#ifndef _ENCODERCODINGPARAMETER_H_
#define _ENCODERCODINGPARAMETER_H_
...
class EncoderConfigLineStr : public JSVM::EncoderConfigLineBase
{
public:
...
class EncoderConfigLineDbl : public JSVM::EncoderConfigLineBase
{
public:
...
class EncoderConfigLineInt : public JSVM::EncoderConfigLineBase
{
public:
...
class EncoderConfigLineUInt : public JSVM::EncoderConfigLineBase
{
public:
...
class EncoderConfigLineChar : public JSVM::EncoderConfigLineBase
{
public:
...
class EncoderCodingParameter : public JSVM::CodingParameter
{
...
protected:
    Bool equals(const Char* str1, const Char* str2, UInt nLetter) {
        return 0 == ::strncmp(str1, str2, nLetter);
    }
    ...
    ErrVal xReadLayerFromFile(std::string& rcFilename,
                              JSVM::LayerParameters& rcLayer);

    ErrVal xReadLine(FILE* hFile,
                     std::string* pacTag);
    ErrVal xReadSliceGroupCfg(JSVM::LayerParameters&  rcLayer);

    ErrVal xReadROICfg(JSVM::LayerParameters&  rcLayer);
                                                   ...
                                                   JSVM::LayerParameters&  rcLayer)
{
    ...
    std::string  cOutputFilename;
    std::string  cMotionFilename;
...
//JVT-Q065 EIDR}
    ...
    for (UInt uiSMIdx = 0; uiSMIdx < 8; uiSMIdx++)
    {
        ...
        {
            xReadScalMat(rcLayer.m_aaucScalingMatrices[uiSMIdx], (uiSMIdx < 6 ? 16 : 64), rcLayer.m_acScalMatFiles[uiSMIdx]);
        }
    }
    ...
    xReadSliceGroupCfg(rcLayer); //Slice group configuration file
...
ErrVal EncoderCodingParameter::xReadSliceGroupCfg (JSVM::LayerParameters&  rcLayer)
{
    UInt mapunit_height;
    UInt mb_height;
    UInt i;
...
ErrVal EncoderCodingParameter::xReadROICfg(JSVM::LayerParameters&  rcLayer)
{
    UInt i;
    FILE* roifile=NULL;
...
#endif //_ENCODERCODINGPARAMETER_H_
