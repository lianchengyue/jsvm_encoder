#ifndef _SCALINGMATRIX_H_
#define _SCALINGMATRIX_H_
...
public:
    ...
    template<UInt uiBufSize> class SeqScaling
    {
    public:
        SeqScaling();
        ErrVal  init (const UChar*          pucBuffer,
...
template<UInt uiBufSize> ScalingMatrix::ScalingList<uiBufSize>::ScalingList()
{
    this->setAll(16); // leszek: C++ standard compliance
}

template<UInt uiBufSize> ErrVal ScalingMatrix::ScalingList<uiBufSize>::init(const UChar* pucBuffer)
{
    ROF(pucBuffer);
...
template<UInt uiBufSize> ErrVal ScalingMatrix::ScalingList<uiBufSize>::read(HeaderSymbolReadIf*  pcReadIf,
                                                                            const UChar* pucScan,
                                                                            Bool&  rbUseDefaultScalingMatrixFlag)
{
    Int   iDeltaScale;
    pcReadIf->getSvlc(iDeltaScale, "SCALING: delta_scale");
...
template<UInt uiBufSize> ErrVal ScalingMatrix::ScalingList<uiBufSize>::write(HeaderSymbolWriteIf*  pcWriteIf,
                                                                             const UChar*  pucScan,
                                                                             const Bool  bUseDefaultScalingMatrixFlag) const
{
    if(bUseDefaultScalingMatrixFlag)
    {
        pcWriteIf->writeSvlc(-8, "SCALING: delta_scale");
        return Err::m_nOK;
    }
    ...
    for(Int n = 0; n < iLastDiff+2; n++)
    {
        ...
        pcWriteIf->writeSvlc(iDeltaScale, "SCALING: delta_scale");
        iLast = this->get(pucScan[n]);
    }
    if(iLastDiff < (Int)this->size() - 2)
    {
        pcWriteIf->writeSvlc(-iLast, "SCALING: delta_scale");
    }
...
template<UInt uiBufSize> Bool ScalingMatrix::ScalingList<uiBufSize>::isSame(const ScalingMatrix::ScalingList<uiBufSize>& rcSL)  const
{
...
template<UInt uiBufSize> ErrVal ScalingMatrix::SeqScaling<uiBufSize>::init(const UChar* pucBuffer, const UChar* pucBufferFallBack)
{
    ROF (pucBuffer);
    m_aiScalingList.init(pucBuffer);
...
template<UInt uiBufSize>
const UChar* ScalingMatrix::SeqScaling<uiBufSize>::getMatrix () const
{
...
template<UInt uiBufSize>
ErrVal ScalingMatrix::SeqScaling<uiBufSize>::read(HeaderSymbolReadIf* pcReadIf,
                                                  const UChar*  pucScan)
{
    pcReadIf->getFlag(m_bScalingListPresentFlag, "SCALING: scaling_list_present_flag");
    ROTRS (!m_bScalingListPresentFlag, Err::m_nOK);

    m_aiScalingList.read(pcReadIf, pucScan, m_bUseDefaultScalingMatrixFlag);
