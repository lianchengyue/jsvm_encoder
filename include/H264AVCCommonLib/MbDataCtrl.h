#ifndef _MBDATACTRL_H_
#define _MBDATACTRL_H_
...
private:
    ErrVal  xInitMb (Int iMbXCurr, Int iMbYCurr);
...
public:
    ...
    ErrVal  copyMotion (MbDataCtrl& rcMbDataCtrl, PicType ePicType = FRAME);
...
public:
    ...
    Void  clear();
    ErrVal init (SliceHeader* pcSliceHeader, MbDataCtrl*  pcMbDataCtrl, Double dLambda);
    ErrVal init (SliceHeader* pcSliceHeader);

    Double getLambda()  { return m_dLambda; }
    SliceHeader* getSliceHeader(PicType ePicType = FRAME)
    {
        return (ePicType==BOT_FIELD) ? m_pcSliceHeaderBot : m_pcSliceHeader;
    }

    MbDataCtrl* getMbDataCtrl()
    {
        return  m_pcMbDataCtrl;
    }
    Bool isInitialized()
    {
        return  m_pcMbDataCtrl != 0;
    }
    ...
    ErrVal setSliceHeader (SliceHeader* pcSliceHeader, PicType ePicType = FRAME)
    {
