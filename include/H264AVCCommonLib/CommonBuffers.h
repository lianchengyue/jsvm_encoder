#ifndef _COMMONBUFFERS_H_
#define _COMMONBUFFERS_H_
...
template<class T, UInt uiSize> class StatBuf
{
public:
    StatBuf()
    {
...
template<class T> class XDataList
{
public:
    XDataList();
    ~XDataList();
...
template<class T>  Void XDataList<T>::reset()
{
    m_uiSize = 0;
    m_uiActive = 0;
}

template<class T>  Void XDataList<T>::copy(XDataList<T>& rcSrc)
{
...
template<class T>  Void XDataList<T>::setActive(UInt ui)
{
    m_uiActive = gMin(ui, m_uiSize);
}

template<class T>  Void XDataList<T>::incActive()
{
...
template<class T>  Void XDataList<T>::decActive()
{
    if(m_uiActive > 0)
    {
        m_uiActive--;
...
template<class T>  UInt XDataList<T>::getSize() const
{
    return m_uiSize;
}

template<class T>  UInt XDataList<T>::getActive() const
{
    return m_uiActive;
}
...
template<class T>  Void XDataList<T>::switchFirst()
{
    T*  pTmp = m_apT[0];
    m_apT[0] = m_apT[1];
    m_apT[1] = pTmp;
