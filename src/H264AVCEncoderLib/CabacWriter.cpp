...
ErrVal CabacWriter::xInitContextModels(const SliceHeader& rcSliceHeader)
{
    ...
    if(bIntra)
    {
        m_cMbTypeCCModel.initBuffer(     (Short*)INIT_MB_TYPE_I,       iQp);
        m_cBlockTypeCCModel.initBuffer(  (Short*)INIT_B8_TYPE_I,       iQp);
        m_cMvdCCModel.initBuffer(        (Short*)INIT_MV_RES_I,        iQp);
        m_cRefPicCCModel.initBuffer(     (Short*)INIT_REF_NO_I,        iQp);
        m_cMotPredFlagCCModel.initBuffer((Short*)INIT_MOTION_PRED_FLAG,iQp);
        m_cResPredFlagCCModel.initBuffer((Short*)INIT_RES_PRED_FLAG,   iQp);
        m_cDeltaQpCCModel.initBuffer(    (Short*)INIT_DELTA_QP_I,      iQp);
        m_cIntraPredCCModel.initBuffer(  (Short*)INIT_IPR_I,           iQp);
        m_cChromaPredCCModel.initBuffer( (Short*)INIT_CIPR_I,          iQp);
        m_cBLSkipCCModel.initBuffer(     (Short*)INIT_BL_SKIP_I,       iQp);

        m_cCbpCCModel.initBuffer(        (Short*)INIT_CBP_I,           iQp);
        m_cBCbpCCModel.initBuffer(       (Short*)INIT_BCBP_I,          iQp);
        m_cMapCCModel.initBuffer(        (Short*)INIT_MAP_I,           iQp);
        m_cLastCCModel.initBuffer(       (Short*)INIT_LAST_I,          iQp);
        m_cOneCCModel.initBuffer(        (Short*)INIT_ONE_I,           iQp);
        m_cAbsCCModel.initBuffer(        (Short*)INIT_ABS_I,           iQp);

        m_cTransSizeCCModel.initBuffer(  (Short*)INIT_TRANSFORM_SIZE_I,iQp);
        m_cFieldFlagCCModel.initBuffer(  (Short*)INIT_MB_AFF_I,        iQp);
        m_cFldMapCCModel.initBuffer(     (Short*)INIT_FLD_MAP_I,       iQp);
        m_cFldLastCCModel.initBuffer(    (Short*)INIT_FLD_LAST_I,      iQp);
    }
    else
    {
        m_cMbTypeCCModel.initBuffer(     (Short*)INIT_MB_TYPE_P        [iIndex], iQp);
        m_cBlockTypeCCModel.initBuffer(  (Short*)INIT_B8_TYPE_P        [iIndex], iQp);
        m_cMvdCCModel.initBuffer(        (Short*)INIT_MV_RES_P         [iIndex], iQp);
        m_cRefPicCCModel.initBuffer(     (Short*)INIT_REF_NO_P         [iIndex], iQp);
        m_cMotPredFlagCCModel.initBuffer((Short*)INIT_MOTION_PRED_FLAG,          iQp);
        m_cResPredFlagCCModel.initBuffer((Short*)INIT_RES_PRED_FLAG,             iQp);
        m_cDeltaQpCCModel.initBuffer(    (Short*)INIT_DELTA_QP_P       [iIndex], iQp);
        m_cIntraPredCCModel.initBuffer(  (Short*)INIT_IPR_P            [iIndex], iQp);
        m_cChromaPredCCModel.initBuffer( (Short*)INIT_CIPR_P           [iIndex], iQp);
        m_cBLSkipCCModel.initBuffer(     (Short*)INIT_BL_SKIP_P,                 iQp);

        m_cCbpCCModel.initBuffer(        (Short*)INIT_CBP_P            [iIndex], iQp);
        m_cBCbpCCModel.initBuffer(       (Short*)INIT_BCBP_P           [iIndex], iQp);
        m_cMapCCModel.initBuffer(        (Short*)INIT_MAP_P            [iIndex], iQp);
        m_cLastCCModel.initBuffer(       (Short*)INIT_LAST_P           [iIndex], iQp);
        m_cOneCCModel.initBuffer(        (Short*)INIT_ONE_P            [iIndex], iQp);
        m_cAbsCCModel.initBuffer(        (Short*)INIT_ABS_P            [iIndex], iQp);

        m_cTransSizeCCModel.initBuffer(  (Short*)INIT_TRANSFORM_SIZE_P [iIndex], iQp);
        m_cFieldFlagCCModel.initBuffer(  (Short*)INIT_MB_AFF_P         [iIndex], iQp);
        m_cFldMapCCModel.initBuffer(     (Short*)INIT_FLD_MAP_P[ iIndex ],       iQp);
        m_cFldLastCCModel.initBuffer(    (Short*)INIT_FLD_LAST_P[ iIndex ],      iQp);
    }
...
ErrVal CabacWriter::init( BitWriteBufferIf *pcBitWriteBufferIf)
{
    ...
    CabaEncoder::init(pcBitWriteBufferIf);
...
ErrVal CabacWriter::startSlice (const SliceHeader& rcSliceHeader)
{
    ...
    xInitContextModels (rcSliceHeader);
    CabaEncoder::start ();
...
{
    CabaEncoder::getLastByte (uiLastByte, uiLastBitPos);
    return Err::m_nOK;
}
ErrVal CabacWriter::setFirstBits (UChar ucByte, UInt uiLastBitPos)
{
    CabaEncoder::setFirstBits (ucByte, uiLastBitPos);
    return Err::m_nOK;
}
//~FIX_FRAG_CAVLC
...
ErrVal CabacWriter::motionPredFlag (MbDataAccess& rcMbDataAccess,
                                    ListIdx eLstIdx)
{
...
ErrVal CabacWriter::xRefFrame (MbDataAccess& rcMbDataAccess,
                               UInt uiRefFrame,
                               ListIdx eLstIdx,
                               ParIdx8x8 eParIdx)
{
    ...
    CabaEncoder::writeSymbol((uiRefFrame==1 ? 0 : 1), m_cRefPicCCModel.get(0, uiCtx));

    if(uiRefFrame > 1)
    {
        CabaEncoder::writeUnarySymbol(uiRefFrame-2, &m_cRefPicCCModel.get(0, 4), 1);
    }
...
ErrVal CabacWriter::xMotionPredFlag(Bool bFlag, ListIdx eLstIdx)
{
    ...
    CabaEncoder::writeSymbol(uiCode, m_cMotPredFlagCCModel.get(0, eLstIdx));
...
ErrVal CabacWriter::blockModes (MbDataAccess& rcMbDataAccess)
{
    ...
    {
        xWriteBlockMode(rcMbDataAccess.getConvertBlkMode(c8x8Idx.b8x8Index()));
    }
    return Err::m_nOK;
}
...
ErrVal CabacWriter::xWriteBlockMode (UInt uiBlockMode)
{
    ...
    if(! m_pcSliceHeader->isBSlice())
    {
        ...
        CabaEncoder::writeSymbol(uiSymbol, m_cBlockTypeCCModel.get(0, 1));
        if(!uiSymbol)
        {
            uiSymbol = (1 == uiBlockMode) ? 0 : 1;
            CabaEncoder::writeSymbol(uiSymbol, m_cBlockTypeCCModel.get(0, 3));
            if(uiSymbol)
            {
                uiSymbol = (2 == uiBlockMode) ? 1:0;
                CabaEncoder::writeSymbol(uiSymbol, m_cBlockTypeCCModel.get(0, 4));
            }
        }
    }
    else
    {
        ...
        CabaEncoder::writeSymbol(uiSymbol, m_cBlockTypeCCModel.get(1, 0));
        if(uiSymbol)
        {
            uiSymbol = (3 > uiBlockMode) ? 0 : 1;
            CabaEncoder::writeSymbol(uiSymbol, m_cBlockTypeCCModel.get(1, 1));
            if(uiSymbol)
            {
                uiSymbol = (7 > uiBlockMode) ? 0 : 1;
                CabaEncoder::writeSymbol(uiSymbol, m_cBlockTypeCCModel.get(1, 2));
                if(uiSymbol)
                {
                    uiBlockMode -= 7;
                    uiSymbol = (uiBlockMode >> 2) & 1;
                    CabaEncoder::writeSymbol(uiSymbol, m_cBlockTypeCCModel.get(1, 3));
                    if(!uiSymbol)
                    {
...
ErrVal CabacWriter::fieldFlag (MbDataAccess& rcMbDataAccess)
{
    ...
    CabaEncoder::writeSymbol(uiSymbol, m_cFieldFlagCCModel.get(0, rcMbDataAccess.getCtxFieldFlag()));
...
ErrVal CabacWriter::skipFlag (MbDataAccess& rcMbDataAccess)
{
    ...
    if(m_pcSliceHeader->isBSlice())
    {
        CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(2, 7 + rcMbDataAccess.getCtxMbSkipped()));
    }
    else
    {
        CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(1, rcMbDataAccess.getCtxMbSkipped()));
    }
...
ErrVal CabacWriter::BLSkipFlag (MbDataAccess& rcMbDataAccess)
{
    ...
    CabaEncoder::writeSymbol(uiSymbol, m_cBLSkipCCModel.get(0, uiCtx));
...
ErrVal CabacWriter::resPredFlag (MbDataAccess& rcMbDataAccess)
{
    ...
    CabaEncoder::writeSymbol(uiSymbol, m_cResPredFlagCCModel.get(0, uiCtx));
...
ErrVal CabacWriter::mbMode (MbDataAccess& rcMbDataAccess)
{
    ...
    if(m_pcSliceHeader->isIntraSlice())
    {
        ...
        CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(0, rcMbDataAccess.getCtxMbIntra4x4()));

        if(uiSymbol)
        {
            uiSymbol = (25 == uiMbMode) ? 1 : 0;
            CabaEncoder::writeTerminatingBit(uiSymbol);

            if(!uiSymbol)
            {
                uiSymbol = (13 > uiMbMode) ? 0 : 1;
                CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(0, 4));
                uiMbMode -= 12* uiSymbol + 1;

                uiSymbol = (4 > uiMbMode) ? 0 : 1;
                CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(0, 5));

                if(uiSymbol)
                {
                    uiSymbol = (8 > uiMbMode) ? 0 : 1;
                    CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(0, 6));
                }

                uiSymbol = (uiMbMode>>1) & 1;
                CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(0, 7));

                uiSymbol = uiMbMode & 1;
                CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(0, 8));
            }
        }
    }
    else
    {
        ...
        if(! m_pcSliceHeader->isBSlice())
        {
            ...
            CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 4));
            if(uiSymbol)
            {
                uiSymbol = (6 == uiMbMode) ? 0 : 1;
                CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 7));
            ...
            {
                uiSymbol = (uiMbMode>>1) & 1;
                CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 5));
                if(uiSymbol)
                {
                ...
                {
                    uiSymbol = 1-(uiMbMode&1);
                    CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 6));
                }
            }
        }
        else
        {
            ...
            CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(2, rcMbDataAccess.getCtxMbType()));
            if(uiSymbol)
            {
                uiSymbol = (3 > uiMbMode) ? 0 : 1;
                CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(2, 4));
                if(uiSymbol)
                {
                    uiSymbol = (11 > uiMbMode) ? 0 : 1;
                    CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(2, 5));
                    if(uiSymbol)
                    {
                        if((uiSymbol = (22 == uiMbMode)) || 11 == uiMbMode)
                        {
                            CabaEncoder::writeSymbol(       1, m_cMbTypeCCModel.get(2, 6));
                            CabaEncoder::writeSymbol(       1, m_cMbTypeCCModel.get(2, 6));
                            CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(2, 6));
                        }
                        else
                        {
                            if(uiMbMode < 23)
        ...
        if(uiIntra16x16Symbol)
        {
            ...
            if(!uiSymbol)
            {
                ...
                CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 8));
                uiIntra16x16Symbol -= (12 * uiSymbol + 1);

                uiSymbol = (4 > uiIntra16x16Symbol) ? 0 : 1;
                CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 9));
                ...
                CabaEncoder::writeSymbol(uiSymbol, m_cMbTypeCCModel.get(1, 10));
...
ErrVal CabacWriter::xWriteMvdComponent (Short sMvdComp, UInt uiAbsSum, UInt uiCtx)
{
    ...
    CabaEncoder::writeSymbol(uiSymbol, m_cMvdCCModel.get(0, uiLocalCtx));
    ROTRS(0 == uiSymbol, Err::m_nOK);
    ...
    CabaEncoder::writeExGolombMvd(sMvdComp-1, &m_cMvdCCModel.get(1, uiCtx), 3);
    CabaEncoder::writeEPSymbol(uiSign);

    return Err::m_nOK;
}


ErrVal CabacWriter::mvd (MbDataAccess& rcMbDataAccess,
                         ListIdx eLstIdx)
{
    Mv cMv = rcMbDataAccess.getMbMvdData(eLstIdx).getMv();
    xWriteMvd(rcMbDataAccess, cMv, B4x4Idx(0), eLstIdx);
    return Err::m_nOK;
}
                         ...
                         ParIdx16x8 eParIdx)
{
    ...
    xWriteMvd(rcMbDataAccess, cMv, B4x4Idx(eParIdx), eLstIdx);
    return Err::m_nOK;
}
                         ...
                         SParIdx8x4 eSParIdx)
{
    ...
    xWriteMvd(rcMbDataAccess, cMv, B4x4Idx(eParIdx+eSParIdx), eLstIdx);
    return Err::m_nOK;
}
...
ErrVal CabacWriter::xWriteMvd (MbDataAccess& rcMbDataAccess,
                               Mv cMv,
                               LumaIdx cIdx,
                               ListIdx eLstIdx)
{
    ...
    xWriteMvdComponent(sHor, cMvA.getAbsHor() + cMvB.getAbsHor(), 0);
    ...
    xWriteMvdComponent(sVer, cMvA.getAbsVer() + cMvB.getAbsVer(), 5);
...
ErrVal CabacWriter::intraPredModeLuma(MbDataAccess& rcMbDataAccess, LumaIdx cIdx)
{
    ...
    CabaEncoder::writeSymbol(iIntraPredModeLuma >= 0 ? 0 : 1, m_cIntraPredCCModel.get(0, 0));
    if(iIntraPredModeLuma >= 0)
    {
        CabaEncoder::writeSymbol((iIntraPredModeLuma & 0x01)     , m_cIntraPredCCModel.get(0, 1));
        CabaEncoder::writeSymbol((iIntraPredModeLuma & 0x02) >> 1, m_cIntraPredCCModel.get(0, 1));
        CabaEncoder::writeSymbol((iIntraPredModeLuma & 0x04) >> 2, m_cIntraPredCCModel.get(0, 1));
    }
...
ErrVal CabacWriter::cbp(MbDataAccess& rcMbDataAccess, UInt uiStart, UInt uiStop)
{
    ...
    CabaEncoder::writeSymbol(uiCbp & 1, m_cCbpCCModel.get(uiCtx, 3 - (a + b)));

    a = uiCbp & 1;
    b = rcMbDataAccess.getAboveLumaCbp(B4x4Idx(2), uiStart, uiStop, true) << 1;

    CabaEncoder::writeSymbol((uiCbp>>1) & 1, m_cCbpCCModel.get(uiCtx, 3 - (a + b)));

    a = rcMbDataAccess.getLeftLumaCbp (B4x4Idx(8), uiStart, uiStop, true);
    b = (uiCbp  << 1) & 2;

    CabaEncoder::writeSymbol((uiCbp>>2) & 1, m_cCbpCCModel.get(uiCtx, 3 - (a + b)));

    a = (uiCbp >> 2) & 1;
    b = uiCbp & 2;

    CabaEncoder::writeSymbol((uiCbp>>3) & 1, m_cCbpCCModel.get(uiCtx, 3 - (a + b)));
    ...
    CabaEncoder::writeSymbol(uiBit, m_cCbpCCModel.get(uiCtx, a + b));
                                   ...
                                   UInt           uiStop)
{
    ...
    xWriteBCbp(rcMbDataAccess, uiNumSig, eResidualMode, cIdx, uiStart, uiStop);

    if(uiNumSig)
    {
        xWriteCoeff(uiNumSig, piCoeff, eResidualMode, pucScan, bFrame, uiStart, uiStop);
    }
    return Err::m_nOK;
}
...
ErrVal CabacWriter::xWriteBCbp(MbDataAccess& rcMbDataAccess,
                               UInt uiNumSig,
                               ResidualMode eResidualMode,
                               LumaIdx cIdx,
                               UInt uiStart,
...
UInt CabacWriter::xGetNumberOfSigCoeff(TCoeff* piCoeff,
                                       ResidualMode eResidualMode,
                                       const UChar* pucScan,
                                       UInt uiStart,
                                       UInt uiStop)
...
{
    CabaEncoder::writeTerminatingBit(uiIsLast);
...
ErrVal CabacWriter::deltaQp (MbDataAccess& rcMbDataAccess)
{
    ...
    CabaEncoder::writeSymbol(uiDQp, m_cDeltaQpCCModel.get(0, uiCtx));
    ...
    if(uiDQp)
    {
        ...
        CabaEncoder::writeUnarySymbol(uiDQp, &m_cDeltaQpCCModel.get(0, 2), 1);
    }
...
ErrVal CabacWriter::samplesPCM(MbDataAccess& rcMbDataAccess)
{
    ...
    CabaEncoder::finish();
    m_pcBitWriteBufferIf->write(1);
    m_pcBitWriteBufferIf->writeAlignZero();
    ...
    m_pcBitWriteBufferIf->pcmSamples(pCoeff, uiSize);
...
ErrVal CabacWriter::transformSize8x8Flag(MbDataAccess& rcMbDataAccess, UInt uiStart, UInt uiStop)
{
    ...
    CabaEncoder::writeSymbol(uiSymbol, m_cTransSizeCCModel.get(0, uiCtx));
                                      ...
                                      UInt          uiStop)
{
    ...
    {
        UInt uiSig = piCoeff[ pucScan[ ui ] ] ? 1 : 0;
        CabaEncoder::writeSymbol(uiSig, rcMapCCModel.get(uiCtxOffset, pos2ctx_map[ui]));

        if(uiSig)
        {
            UInt uiLast = (++uiCodedSig == uiNumSig ? 1 : 0);
            CabaEncoder::writeSymbol(uiLast, rcLastCCModel.get(uiCtxOffset, pos2ctx_last8x8[ui]));
    ...
    while((ui--) != uiStart)
    {
        ...
        if(iCoeff)
        {
            ...
            UInt uiCtx    = gMin (c1, 4);
            UInt uiSymbol = uiAbs > 1 ? 1 : 0;
            CabaEncoder::writeSymbol(uiSymbol, m_cOneCCModel.get(uiCtxOffset, uiCtx));
            ...
            if(uiSymbol)
            {
                ...
                c1     = 0;
                c2++;
                CabaEncoder::writeExGolombLevel(uiAbs, m_cAbsCCModel.get(uiCtxOffset, uiCtx));
            }
            else if(c1)
            {
                c1++;
