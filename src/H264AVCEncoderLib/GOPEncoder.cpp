...
ErrVal LayerEncoder::create(LayerEncoder*& rpcLayerEncoder)
{
                          ...
                          )
{
    ...
    xInitCodingOrder(m_uiDecompositionStages, m_uiTemporalResolution, m_uiFrameDelay);
...
ErrVal LayerEncoder::xInitCodingOrder(UInt uiDecompositionStages, UInt uiTemporalSubSampling, UInt uiMaxFrameDelay)
{
...
ErrVal LayerEncoder::addParameterSetBits(UInt uiParameterSetBits)
{
...
ErrVal LayerEncoder::xCreateData(const SequenceParameterSet& rcSPS)
{
    UInt uiIndex;
    ...
    {
        Frame::create(m_papcFrame[uiIndex], *m_pcYuvFullPelBufferCtrl, *m_pcYuvHalfPelBufferCtrl, FRAME, 0);
        m_papcFrame[uiIndex]->init();

        if(m_papcELFrame)
        {
            Frame::create(m_papcELFrame[uiIndex], *m_pcYuvFullPelBufferCtrl, *m_pcYuvHalfPelBufferCtrl, FRAME, 0);
            m_papcELFrame[uiIndex]->init();
        }
    }

    for(uiIndex = 0; uiIndex < NUM_TMP_FRAMES;  uiIndex++)
    {
        Frame::create(m_apcFrameTemp[uiIndex], *m_pcYuvFullPelBufferCtrl, *m_pcYuvHalfPelBufferCtrl, FRAME, 0);
        m_apcFrameTemp[uiIndex] ->init();
    }

    Frame::create(m_pcResidualLF, *m_pcYuvFullPelBufferCtrl, *m_pcYuvHalfPelBufferCtrl, FRAME, 0);
    m_pcResidualLF->init();

    Frame::create(m_pcResidualILPred, *m_pcYuvFullPelBufferCtrl, *m_pcYuvHalfPelBufferCtrl, FRAME, 0);
    m_pcResidualILPred->init();

    Frame::create(m_pcSubband, *m_pcYuvFullPelBufferCtrl, *m_pcYuvHalfPelBufferCtrl, FRAME, 0);
    m_pcSubband->init();

    Frame::create(m_apcBaseFrame[0], *m_pcYuvFullPelBufferCtrl, *m_pcYuvHalfPelBufferCtrl, FRAME, 0);
    Frame::create(m_apcBaseFrame[1], *m_pcYuvFullPelBufferCtrl, *m_pcYuvHalfPelBufferCtrl, FRAME, 0);
    m_apcBaseFrame[0]->init();
    m_apcBaseFrame[1]->init();

    Frame::create(m_pcAnchorFrameOriginal, *m_pcYuvFullPelBufferCtrl, *m_pcYuvHalfPelBufferCtrl, FRAME, 0);
    m_pcAnchorFrameOriginal->init();

    Frame::create(m_pcAnchorFrameReconstructed , *m_pcYuvFullPelBufferCtrl, *m_pcYuvHalfPelBufferCtrl, FRAME, 0);
    m_pcAnchorFrameReconstructed->init();

    Frame::create(m_pcBaseLayerFrame           , *m_pcYuvFullPelBufferCtrl, *m_pcYuvHalfPelBufferCtrl, FRAME, 0);
    m_pcBaseLayerFrame->init();

    Frame::create(m_pcBaseLayerResidual        , *m_pcYuvFullPelBufferCtrl, *m_pcYuvHalfPelBufferCtrl, FRAME, 0);
    m_pcBaseLayerResidual->init();
    ...
    for(uiIndex = 0; uiIndex <= m_uiMaxGOPSize; uiIndex++)
    {
      ...
      if(m_pcLayerParameters->getPAff())
      {
        ...
        m_pacControlData[uiIndex].setSliceHeader(pcSliceHeader, BOT_FIELD);
      }
    }
...
ErrVal LayerEncoder::xDeleteData()
{
    UInt uiIndex;
    ...
    if(m_papcFrame)
    {
        ...
        //JVT-R057 LA-RDO}
                 m_papcFrame[uiIndex]->uninit();
                 m_papcFrame[uiIndex]->destroy();
    ...
    if(m_papcELFrame)
    {
        for(uiIndex = 0; uiIndex <= m_uiMaxGOPSize; uiIndex++)
        {
            ...
            {
                m_papcELFrame[uiIndex]->uninit();
                m_papcELFrame[uiIndex]->destroy();
    ...
    if(m_pcResidualLF)
    {
        m_pcResidualLF->uninit();
        m_pcResidualLF->destroy();
        m_pcResidualLF = 0;
    }
    if(m_pcResidualILPred)
    {
        m_pcResidualILPred->uninit();
        m_pcResidualILPred->destroy();
        m_pcResidualILPred = 0;
    }
    if(m_pcSubband)
    {
    ...
    for(uiIndex = 0; uiIndex < NUM_TMP_FRAMES; uiIndex++)
    {
        ...
        {
            m_apcFrameTemp[uiIndex]->uninit();
            m_apcFrameTemp[uiIndex]->destroy();
    ...
    for(UInt uiBaseIdx = 0; uiBaseIdx < 2; uiBaseIdx++)
    {
        if(m_apcBaseFrame[uiBaseIdx])
        {
            ...
            // JVT-R057 LA-RDO}
            m_apcBaseFrame[uiBaseIdx]->uninit();
            m_apcBaseFrame[uiBaseIdx]->destroy();
    ...
    {
        m_pcAnchorFrameOriginal->uninit();
        m_pcAnchorFrameOriginal->destroy();
        m_pcAnchorFrameOriginal = 0;
    }

    if(m_pcAnchorFrameReconstructed)
    {
        m_pcAnchorFrameReconstructed->uninit();
        m_pcAnchorFrameReconstructed->destroy();
        m_pcAnchorFrameReconstructed = 0;
    }

    if(m_pcBaseLayerFrame)
    {
        m_pcBaseLayerFrame->uninit();
        m_pcBaseLayerFrame->destroy();
        m_pcBaseLayerFrame = 0;
    }

    if(m_pcBaseLayerResidual)
    {
        m_pcBaseLayerResidual->uninit();
        m_pcBaseLayerResidual->destroy();
        m_pcBaseLayerResidual = 0;
    }
...
ErrVal LayerEncoder::xFillAndUpsampleFrame(Frame* pcFrame, PicType ePicType, Bool bFrameMbsOnlyFlag)
{
    m_pcYuvFullPelBufferCtrl->initMb();
    m_pcYuvHalfPelBufferCtrl->initMb();
...
ErrVal LayerEncoder::xFillAndExtendFrame(Frame* pcFrame, PicType ePicType, Bool bFrameMbsOnlyFlag)
{
    m_pcYuvFullPelBufferCtrl->initMb();

    pcFrame->extendFrame(NULL, ePicType, bFrameMbsOnlyFlag);
...
ErrVal LayerEncoder::xInitExtBinDataAccessor(ExtBinDataAccessor& rcExtBinDataAccessor)
{
...
ErrVal LayerEncoder::xAppendNewExtBinDataAccessor(ExtBinDataAccessorList& rcExtBinDataAccessorList,
...
ErrVal LayerEncoder::xEncodeLayerRepresentation(ExtBinDataAccessorList& rcOutExtBinDataAccessorList,
                                                 ...
                                                 PicType                 ePicType)
{
    ...
    {
        xWriteSEI(rcOutExtBinDataAccessorList, *pcSliceHeader, uiBitsSEI);
    }
    ...
    for(Int iSliceGroupID = pcFMO->getFirstSliceGroupId(); ! pcFMO->SliceGroupCompletelyCoded(iSliceGroupID); iSliceGroupID = pcFMO->getNextSliceGroupId(iSliceGroupID))
    {
        ...
        while(!rcControlData.m_bSliceGroupAllCoded)
        {
            ...
            {
                m_pcSliceEncoder->encodeMbAffSliceSVC(rcControlData, *pcOrgFrame, *pcFrame, pcResidualLF, pcResidualILPred, pcPredSignal,
            ...
            //----- close NAL UNIT -----
            UInt auiBits[16];
            m_pcNalUnitEncoder->closeAndAppendNalUnits(auiBits,
...
ErrVal LayerEncoder::xInitRefFrameLists(UInt uiFrameIdInGOP, UInt uiTemporalId)
{
...
ErrVal LayerEncoder::xSetReorderingCommandsST(RefPicListReOrdering& rcReOrdering,
...
ErrVal LayerEncoder::xSetReorderingCommandsLT(RefPicListReOrdering& rcReOrdering,
...
ErrVal LayerEncoder::xSetDecRefPicMarkingST(DecRefPicMarking& rcDecRefPicMarking,
                                             UIntList&         cPicNumList,
                                             UInt              uiCurrPicNum,
                                             UInt              uiMaxPicNum)
{
...
ErrVal LayerEncoder::xSetDecRefPicMarkingLT(DecRefPicMarking& rcDecRefPicMarking,
...
ErrVal LayerEncoder::xSetSliceTypeAndRefLists(SliceHeader& rcSliceHeader, UInt uiFrameIdInGOP, PicType ePicType)
{
    ...
    if(m_bUseLongTermPics)
    {
        xSetReorderingCommandsLT(rcSliceHeader.getRefPicListReorderingL0(), cPicNumList0);
        xSetReorderingCommandsLT(rcSliceHeader.getRefPicListReorderingL1(), cPicNumList1);
    }
    else
    {
        ...
        xSetReorderingCommandsST(rcSliceHeader.getRefPicListReorderingL0(), cPicNumList0, uiCurrPicNum, !bSendReOrdering);
        xSetReorderingCommandsST(rcSliceHeader.getRefPicListReorderingL1(), cPicNumList1, uiCurrPicNum, !bSendReOrdering);
    }
    return Err::m_nOK;
}

ErrVal LayerEncoder::xSetSliceHeaderBase(SliceHeader& rcSliceHeader, UInt uiFrameIdInGOP, PicType ePicType)
{
...
ErrVal LayerEncoder::xSetMMCOAndUpdateParameters(SliceHeader& rcSliceHeader, UInt uiFrameIdInGOP, PicType ePicType)
{
    ROF(m_auiFrameIdToFrameNumOrLTIdx[uiFrameIdInGOP] == MSYS_UINT_MAX); // frame_num must not be set
    UInt        uiTemporalId    = m_auiFrameIdToTemporalId[uiFrameIdInGOP];
    Bool        bFieldPic       = (ePicType != FRAME);
    ...
    if(! m_bUseLongTermPics && uiFrameIdInGOP == m_uiFrameIdWithSmallestTId && bRefFrmOr2ndFld)
    {
        if(bUseRefBasePic && m_bMMCOBaseEnable)
        {
            ...
            xSetDecRefPicMarkingST(rcDecRefBasePicMarking, cBasePicNumList, uiCurrPicNum, uiMaxPicNum);
        }
        if(m_bMMCOEnable)
        {
          UIntList cPicNumList;
          ...
          xSetDecRefPicMarkingST(rcDecRefPicMarking, cPicNumList, uiCurrPicNum, uiMaxPicNum);
        }
    }
    ...
    if(m_bUseLongTermPics && bRefPic && !bIdrFlag)
    {
        ...
        xSetDecRefPicMarkingLT(rcDecRefPicMarking, (UInt)iLongTermFrmIdx, bSendMaxLongTerm, m_iMaxLongTermFrmIdx);
    }
...
ErrVal LayerEncoder::xInitSliceHeader(UInt uiFrameIdInGOP, PicType ePicType)
{
    ...
    xSetSliceHeaderBase         (*pcSliceHeader, uiFrameIdInGOP, ePicType);
    xSetSliceTypeAndRefLists    (*pcSliceHeader, uiFrameIdInGOP, ePicType);
    xSetMMCOAndUpdateParameters (*pcSliceHeader, uiFrameIdInGOP, ePicType);
...
ErrVal LayerEncoder::xInitSliceHeadersAndRefLists()
{
    ...
    for(; uiCodingIndex <= (1U << MAX_DSTAGES); uiCodingIndex++)
    {
        ...
        if(uiFrameIdInGOP <= m_uiGOPSize)
        {
            UInt uiTemporalId  = m_auiFrameIdToTemporalId  [uiFrameIdInGOP];
            //----- reference lists -----
            xInitRefFrameLists(uiFrameIdInGOP, uiTemporalId);
            //----- slice headers -----
            xPAffDecision(uiFrameIdInGOP);
            if(m_pbFieldPicFlag[uiFrameIdInGOP])
            {
                xInitSliceHeader(uiFrameIdInGOP, (m_bBotFieldFirst ? BOT_FIELD : TOP_FIELD));
                xInitSliceHeader(uiFrameIdInGOP, (m_bBotFieldFirst ? TOP_FIELD : BOT_FIELD));
            }
            else
            {
...
ErrVal LayerEncoder::xInitGOP(PicBufferList&  rcPicBufferInputList)
{
    ...
    for(; uiFrame <= m_uiGOPSize; uiFrame++, cInputIter++)
    {
        if(m_pcResizeParameters->m_iExtendedSpatialScalability == ESS_PICT)
        {
            m_pcResizeParameters->readPictureParameters(m_pESSFile, ! m_bInterlaced);
        }
...
ErrVal LayerEncoder::xInitBitCounts()
{
...
UInt LayerEncoder::getPicCodingType(UInt uiTemporalId, UInt uiFrmIdInTLayer)
{
...
ErrVal LayerEncoder::updateMaxSliceSize(UInt uiAUIndex, UInt& ruiMaxSliceSize)
{
...
Bool LayerEncoder::isMGSEnhancementLayer(UInt& ruiLevelIdc)
{
...
ErrVal LayerEncoder::getBaseLayerLevelIdc(UInt& uiLevelIdc, Bool& bBiPred8x8Disable, Bool& bMCBlks8x8Disable)
{
    uiLevelIdc                 = 0;
...
ErrVal LayerEncoder::getBaseLayerStatus(Bool&   bExists,
                                         PicType ePicType,
                                         UInt    uiTemporalId)
{
    bExists = false;
...
ErrVal LayerEncoder::getBaseLayerDataAvlb(Frame*&       pcFrame,
                                           Frame*&       pcResidual,
...
ErrVal LayerEncoder::getBaseLayerData(SliceHeader&  rcELSH,
                                Frame*&       pcFrame,
                                Frame*&       pcResidual,
                                MbDataCtrl*&  pcMbDataCtrl,
                                ...
                                UInt              uiTemporalId)
{
    ...
    {
        m_apcFrameTemp[0]->copy(pcFrame, ePicType);
        //JVT-R057 LA-RDO{
        if(m_bLARDOEnable) {
...
ErrVal LayerEncoder::xSetBaseLayerData(UInt    uiFrameIdInGOP,
                                        PicType ePicType)
{
        ...
        (pcSliceHeader->getSPS().getConvertedLevelIdc() >= 30 || m_bBiPred8x8Disable))
    {
        ...
        m_pcH264AVCEncoder->getBaseLayerLevelIdc(uiBaseLayerId,
                                                 uiBaseLevel,
                                                 bBaseBiPred8Disable,
                                                 bBaseMCBlks8x8Disable);
...
Frame* LayerEncoder::getMGSLPRec(UInt uiLowPassIndex)
{
...
Frame* LayerEncoder::xGetRefFrame(UInt         uiRefIndex,
...
ErrVal LayerEncoder::xClearELPics()
{
...
ErrVal LayerEncoder::xUpdateELPics()
{
    ROFRS(m_papcELFrame, Err::m_nOK);
    ...
    for(UInt uiIndex = 0; uiIndex <= m_uiGOPSize; uiIndex++)
    {
        if(m_papcELFrame[uiIndex]->isUnvalid())
        {
            ...
            if(pcELPic)
            {
                m_papcELFrame[uiIndex]->copy(const_cast<Frame*>(pcELPic), FRAME);
                xFillAndUpsampleFrame(m_papcELFrame[uiIndex], FRAME, m_bFrameMbsOnlyFlag);
...
Frame* LayerEncoder::getRefPic(UInt uiTemporalId, UInt uiFrameIdInTId)
{
...
ErrVal LayerEncoder::xSetScalingFactors()
{
    for(UInt uiLevel = 0; uiLevel < m_uiDecompositionStages; uiLevel++)
    {
        xSetScalingFactors(uiLevel);
    }
    return Err::m_nOK;
}


ErrVal LayerEncoder::xSetScalingFactors(UInt uiBaseLevel)
{
...
ErrVal LayerEncoder::xClearBufferExtensions()
{
    for(UInt uiFrame = 0; uiFrame <= m_uiGOPSize; uiFrame++)
    {
        m_papcFrame[uiFrame]  ->uninitHalfPel();
        m_papcFrame[uiFrame]  ->removeFrameFieldBuffer();
        if(m_papcELFrame)
        {
            m_papcELFrame[uiFrame]->uninitHalfPel();
            m_papcELFrame[uiFrame]->removeFrameFieldBuffer();
        }
    }

    m_pcSubband           ->uninitHalfPel();
    m_pcSubband           ->removeFrameFieldBuffer();
    m_pcResidualLF        ->uninitHalfPel();
    m_pcResidualLF        ->removeFrameFieldBuffer();
    m_pcResidualILPred    ->uninitHalfPel();
    m_pcResidualILPred    ->removeFrameFieldBuffer();
    m_apcBaseFrame[0]     ->uninitHalfPel();
    m_apcBaseFrame[0]     ->removeFrameFieldBuffer();
    m_apcBaseFrame[1]     ->uninitHalfPel();
    m_apcBaseFrame[1]     ->removeFrameFieldBuffer();
    return Err::m_nOK;
}

ErrVal LayerEncoder::xGetAndSetPredictionLists(ControlData& rcControlData,
                                                UInt         uiFrameIdInGOP,
                                                PicType      ePicType,
                                                Bool         bHalfPel)
{
    ...
    else
    {
        ...
        //flq, predictionLists
        xGetPredictionLists(rcRefList0ME, rcRefList1ME, uiFrameIdCol, uiFrameIdInGOP, ePicType, RLU_MOTION_ESTIMATION, bHalfPel);
    ...
    if(pcSliceHeader->isMbaffFrame())
    {
        ...
        gSetFrameFieldLists(cRefList0Top, cRefList0Bot, rcRefList0);
        gSetFrameFieldLists(cRefList1Top, cRefList1Bot, rcRefList1);
...
ErrVal LayerEncoder::xGetPredictionLists(RefFrameList&  rcRefList0,
                                   ...
                                   Bool           bHalfPel)
{
    ...
    if(b2ndField)
    {
      ...
      if(!pcFrame->isExtended())
      {
        if(bHalfPel)
        {
            xFillAndUpsampleFrame(pcFrame, FRAME, bFrameMbsOnlyFlag);
        }
        else
        {
            xFillAndExtendFrame(pcFrame, FRAME, bFrameMbsOnlyFlag);
        }
      }
      rcRefList0.add(pcFrame->getPic(eOppositePicType));
    ...
    for(UIntList::iterator iter0 = rcFrameIdList0.begin(); iter0 != rcFrameIdList0.end(); iter0++)
    {
      ...
      if(bFieldPic)
      {
          rcRefList0.add(pcFrame->getPic(ePicType));
          rcRefList0.add(pcFrame->getPic(eOppositePicType));
      }
      else
      {
          rcRefList0.add(pcFrame->getPic(FRAME));
      }
      pcFrame->setLongTerm(m_bUseLongTermPics);
    }
    //===== L1 list =====
    for(UIntList::iterator iter1 = rcFrameIdList1.begin(); iter1 != rcFrameIdList1.end(); iter1++)
    {
        ...
        if(bFieldPic)
        {
            rcRefList1.add(pcFrame->getPic(ePicType));
            rcRefList1.add(pcFrame->getPic(eOppositePicType));
        }
        else
        {
            rcRefList1.add(pcFrame->getPic(FRAME));
        }
        pcFrame->setLongTerm(m_bUseLongTermPics);
...
ErrVal LayerEncoder::xInitBaseLayerData(ControlData&  rcControlData,
                                  PicType       ePicType)
{
    //===== init =====
    ...
    {
        m_pcH264AVCEncoder->getBaseLayerDataAvlb(pcBaseFrame,
                                                  pcBaseResidual,
                                                  pcBaseDataCtrl,
        ...
        if(bBaseDataAvailable)
        {
            ...
            m_pcH264AVCEncoder->getBaseLayerData(*pcSliceHeader,
                                                  pcBaseFrame,
                                                  pcBaseResidual,
                                                  pcBaseDataCtrl,
    ...
    if(pcBaseDataCtrl)
    {
        ...
        m_pcBaseLayerCtrl->upsampleMotion(pcSliceHeader, m_pcResizeParameters, pcBaseDataCtrl, pcRefFrameList0, pcRefFrameList1, m_pcResizeParameters->m_bFieldPicFlag, (m_uiEssRPChkEnable!=0), m_uiMVThres);
        ...
        {
            m_pcBaseLayerCtrlField->initSlice(*pcSliceHeader, PRE_PROCESS, false, NULL);
            m_pcBaseLayerCtrlField->upsampleMotion(pcSliceHeader, m_pcResizeParameters, pcBaseDataCtrl, pcRefFrameList0, pcRefFrameList1, true, (m_uiEssRPChkEnable!=0), m_uiMVThres);
    ...
    if(bBaseDataAvailable)
    {
        ...
        m_pcBaseLayerFrame->intraUpsampling(pcBaseFrame, pcTempBaseFrame, pcTempFrame, m_cDownConvert, m_pcResizeParameters,
...
ErrVal LayerEncoder::xInitControlData(UInt    uiFrameIdInGOP,
                                UInt    uiTemporalId,
                                PicType ePicType)
{
    ControlData&  rcControlData = m_pacControlData[uiFrameIdInGOP];
    ...
    xGetAndSetPredictionLists(rcControlData, uiFrameIdInGOP, ePicType, true);
...
ErrVal LayerEncoder::xGetInitialOutputDelay(UInt& ruiInitDelay, UInt uiTemporalLevel)
{
...
ErrVal LayerEncoder::xSetCodingNumbers(UInt uiCodingIndex)
{
    ...
    if(uiCodingIndex == 0) // first frame in sequence: reset all
    {
        for(UInt uiTId = 0; uiTId <= MAX_DSTAGES; uiTId++)
        {
            ...
            xGetInitialOutputDelay(m_auiInitialDelay[uiTId], uiTId);
        }
        return Err::m_nOK;
    }
    UInt  uiFrameIdInGop  = m_auiCodingIndexToFrameId [uiCodingIndex  ];
...
ErrVal LayerEncoder::xGetRelativeOutputDelay(UInt& ruiDelay, UInt uiFrameIdInGOP, UInt uiTemporalLevel)
{
...
ErrVal LayerEncoder::xEncodePicture (Bool& rbPictureCoded,
                                     UInt  uiTemporalId,
                                     UInt  uiFrameIdInGOP,
                                     ...
                                     PicBufferList&  rcPicBufferInputList)
{
    ...
    if(uiFrameIdInGOP == 0 && m_uiGOPNumber)
    {
      //====== don't code first anchor picture if it was coded within the last GOP =====
        m_papcFrame[uiFrameIdInGOP]->copyAll(m_pcAnchorFrameReconstructed);
        m_apcBaseFrame[0]->copyAll(m_apcBaseFrame[1]);
        m_apcBaseFrame[0]->copyPicParameters(*m_apcBaseFrame[1]);
        // JVT-R057 LA-RDO{
        if(m_bLARDOEnable)
        {
    ...
    for(UInt uiPicType = 0; uiPicType < uiNumPics; uiPicType++)
    {
        ...
        if(pcSliceHeader->getIdrFlag() && m_bEnableHrd)
        {
            ...
            for(UInt uiLayer = 0; uiLayer < m_pcSPS->getVUI()->getNumLayers(); uiLayer++)
            {
                ...
                {
                    m_apcScheduler->get(uiIndex)->createBufferingSei(pcBufferingPeriodSei, m_pcParameterSetMng, (uiDependencyId<<4)+uiQualityLevel);
                    SEI::MessageList cSEIMessageList;
                    cSEIMessageList.push_back(pcBufferingPeriodSei);
                    xWriteSEI(rcBPSEIOutputList, cSEIMessageList, uiBits);
                }
            }
            // nested
            for(UInt uiLayer = 0; uiLayer < m_pcSPS->getVUI()->getNumLayers(); uiLayer++)
            {
                ...
                {
                    m_apcScheduler->get(uiIndex)->createBufferingSei(pcBufferingPeriodSei, m_pcParameterSetMng, (uiDependencyId<<4)+uiQualityLevel);
                    xWriteNestingSEIforHrd(rcBPSEIOutputList, pcBufferingPeriodSei, uiDependencyId, uiQualityLevel, uiTemporalLevel, uiBits);
                }
            }
        }
        if(m_bEnableHrd)
        {
            ...
            for(UInt uiLayer = 0; uiLayer < m_pcSPS->getVUI()->getNumLayers(); uiLayer++)
            {
                ...
                if(uiTemporalLevel >= uiTemporalId)
                {
                    ...
                    {
                      UInt uiDpbDelay = 0;
                      xGetRelativeOutputDelay(uiDpbDelay, uiFrameIdInGOP, uiTemporalLevel);
                      m_apcScheduler->get(uiIndex)->createTimingSei(pcPicTimingSei, m_pcSPS->getVUI(), uiDpbDelay, *pcSliceHeader, bFieldCoded, uiLayer);
                      SEI::MessageList  cSEIMessageList;
                      cSEIMessageList.push_back(pcPicTimingSei);
                      xWriteSEI(rcSEIOutputList,cSEIMessageList,uiBits);
                    }
                }
            }
            // nested
            for(UInt uiLayer = 0; uiLayer < m_pcSPS->getVUI()->getNumLayers(); uiLayer++)
            {
                ...
                if(uiTemporalLevel >= uiTemporalId)
                {
                    if(uiDependencyId != 0 || uiQualityLevel != 0)
                    {
                        ...
                        xGetRelativeOutputDelay(uiDpbDelay, uiFrameIdInGOP, uiTemporalLevel);
                        m_apcScheduler->get(uiIndex)->createTimingSei(pcPicTimingSei, m_pcSPS->getVUI(), uiDpbDelay, *pcSliceHeader, bFieldCoded, uiLayer);
                        xWriteNestingSEIforHrd(rcSEIOutputList, pcPicTimingSei, uiDependencyId, uiQualityLevel, uiTemporalLevel, uiBits);
                    }
                }
            }
        }
        ...
        if(m_pcCodingParameter->getTl0DepRepIdxSeiEnable())
        {
            ...
            if(!uiQualityId)
            {
                if(uiDependencyId)
                {
                    writeNestingTl0DepRepIdxSEIMessage(rcSEIOutputList, uiTemporalId, uiDependencyId, m_uiTl0DepRepIdx, m_uiLastIdrPicId, uiBits);
                }
                else
                {
                    writeTl0DepRepIdxSEI(rcSEIOutputList, m_uiTl0DepRepIdx, m_uiLastIdrPicId, uiBits);
                }
            }
        }
        ...
        if(m_pcCodingParameter->getNestingSEIEnable() && m_pcCodingParameter->getSceneInfoEnable())
        {
            ...
            if(!uiQualityId)
            {
                writeNestingSceneInfoSEIMessage(rcSEIOutputList, uiTemporalId, uiDependencyId, uiBits);
            }
        }
        ...
        {
            xWriteRedundantKeyPicSEI(rcSEIOutputList, uiBits);
        }
        // JVT-W049 }
        ...
        {
            pcRCFrame->copyAll(pcFrame);
            if(rcControlData.getBaseLayerSbb())
                pcRCBLResidual->copyAll(rcControlData.getBaseLayerSbb());
            if(rcControlData.getBaseLayerRec())
                pcRCBLIntraRec->copyAll(rcControlData.getBaseLayerRec());
        }
        ...
        //===== encode primary picture =====
        {
            ...
            xEncodeLayerRepresentation(rcOutputList, rcControlData,
            ...
            m_pcSubband->copy(pcFrame, ePicType);
        ...
        if (pcSliceHeader->getPPS().getRedundantPicCntPresentFlag() && (uiTemporalId == 0 || !pcSliceHeader->getPPS().getRedundantKeyPicCntPresentFlag()))
        {
            ...
            for(uiRedundantPicCnt = 1; uiRedundantPicCnt <= uiRedundantPicNum; uiRedundantPicCnt++)
            {
                ...
                xEncodeLayerRepresentation(rcRedOutputList, rcControlData,
        ...
        if(uiTemporalId == 0)
        {
            m_apcBaseFrame[uiFrameIdInGOP>>m_uiDecompositionStages]->copy(pcFrame, ePicType);
    ...
    if(uiTemporalId == 0)
    {
        xUpdateLowPassRec(uiFrameIdInGOP >> m_uiDecompositionStages);
    }

    //===== finish =====
    UInt  uiBaseLevel         = m_uiDecompositionStages - uiTemporalId;
    UInt  uiFrameIdInLevel    = uiFrameIdInGOP >> uiBaseLevel;
    xCalculateAndAddPSNR(uiBaseLevel, uiFrameIdInLevel, rcPicBufferInputList, cPicOutputDataList);
    xCalculateTiming    (cPicOutputDataList, uiFrameIdInLevel);
    xOutputPicData      (cPicOutputDataList);
    ...
    {
        m_pcAnchorFrameReconstructed->copyAll(m_papcFrame[m_uiGOPSize]);
    }
    return Err::m_nOK;
}
...
ErrVal LayerEncoder::xUpdateLowPassRec(UInt uiLowPassIndex)
{
    ...
    ROF  (pcLPRec);
    m_apcBaseFrame[uiLowPassIndex]->copy(pcLPRec, FRAME);
    xFillAndExtendFrame(m_apcBaseFrame[uiLowPassIndex], FRAME, m_pcSPS->getFrameMbsOnlyFlag());
    return Err::m_nOK;
}
...
ErrVal LayerEncoder::xOutputPicData(PicOutputDataList& rcPicOutputDataList)
{
...
ErrVal LayerEncoder::xStoreReconstruction(PicBufferList& rcPicBufferOutputList)
{
...
ErrVal LayerEncoder::initGOP(AccessUnitData& rcAccessUnitData,
                       PicBufferList&  rcPicBufferInputList)
{
    ROT   (m_bGOPInitialized);
    ROTRS (rcPicBufferInputList.empty(), Err::m_nOK);
    xInitGOP(rcPicBufferInputList);
    xSetScalingFactors();
    xClearELPics();
    ::memset(m_abCoded, 0x00, sizeof(m_abCoded));
    if(m_bFirstGOPCoded)
...
ErrVal LayerEncoder::process(UInt             uiAUIndex,
                              AccessUnitData&  rcAccessUnitData,
                              PicBufferList&   rcPicBufferInputList,
                              ...
                              ParameterSetMng* pcParameterSetMng)
{
    ...
    UInt   uiFrameIdInGOP  = m_auiCodingIndexToFrameId[uiCodingIndex ];
    ROTRS(uiFrameIdInGOP  > m_uiGOPSize, Err::m_nOK);
    xSetCodingNumbers(uiCodingIndex);
    UInt   uiTemporalId    = m_auiFrameIdToTemporalId [uiFrameIdInGOP];
    ...
    if(--m_uiNumFramesLeftInGOP == 0)
    {
        ...
        xFinishGOP          (rcPicBufferInputList,
...
ErrVal LayerEncoder::xCalculateAndAddPSNR(UInt               uiStage,
                                   UInt               uiFrame,
                                   PicBufferList&     rcPicBufferInputList,
                                   PicOutputDataList& rcPicOutputDataList)
{
...
ErrVal LayerEncoder::finish(UInt&    ruiNumCodedFrames,
                     Double&  rdOutputRate,
...
ErrVal LayerEncoder::xWriteSEI (ExtBinDataAccessorList& rcOutExtBinDataAccessorList, SliceHeader& rcSH, UInt& ruiBit)
{
    UInt uiBit = 0;
    ...
    if(m_bWriteSEI)
    {
        ...
        SEI::SubSeqInfo::create(pcSubSeqInfo);
        ...
        pcSubSeqInfo->init(uiSubSeqLayer, 0, bFirstRefPicFlag, bLeadingNonRefPicFlag);

        m_pcNalUnitEncoder->write(cSEIMessageList);

        m_pcNalUnitEncoder->closeNalUnit(uiBit);
        xAppendNewExtBinDataAccessor(rcOutExtBinDataAccessorList, &m_cExtBinDataAccessor);
        uiBit += 4*8;
        ruiBit += uiBit;
    }
...
ErrVal LayerEncoder::xWritePrefixUnit(ExtBinDataAccessorList& rcOutExtBinDataAccessorList, SliceHeader& rcSH, UInt& ruiBit)
{
    UInt uiBit = 0;
...
//NonRequired JVT-Q066 (06-04-08){{
ErrVal LayerEncoder::xSetNonRequiredSEI(SliceHeader* pcSliceHeader, SEI::NonRequiredSei* pcNonRequiredSei)
{
...
//JVT-W052
ErrVal LayerEncoder::xWriteIntegrityCheckSEI(ExtBinDataAccessorList &rcOutExtBinDataAccessorList, SEI::SEIMessage *pcSEIMessage, UInt &ruiBits)
{
    UInt uiBits = 0;
    xInitExtBinDataAccessor(m_cExtBinDataAccessor);
    m_pcNalUnitEncoder->initNalUnit(&m_cExtBinDataAccessor);
...
// JVT-W062 {
ErrVal LayerEncoder::writeNestingTl0DepRepIdxSEIMessage(ExtBinDataAccessorList &rcOutExtBinDataAccessorList, UInt uiTId, UInt uiDId, UInt uiTl0DepRepIdx, UInt uiEfIdrPicId, UInt& ruiBits)
{
    SEI::ScalableNestingSei* pcScalableNestingSei;
    SEI::ScalableNestingSei::create(pcScalableNestingSei);
    ...
    xInitExtBinDataAccessor(m_cExtBinDataAccessor);
    m_pcNalUnitEncoder->initNalUnit(&m_cExtBinDataAccessor);
    m_pcNalUnitEncoder->writeScalableNestingSei(cSEIMessageList);
    m_pcNalUnitEncoder->closeNalUnit(uiBits);
    xAppendNewExtBinDataAccessor(rcOutExtBinDataAccessorList, &m_cExtBinDataAccessor);
...
ErrVal LayerEncoder::writeTl0DepRepIdxSEI(ExtBinDataAccessorList &rcOutExtBinDataAccessorList, UInt uiTl0DepRepIdx, UInt uiEfIdrPicId, UInt& ruiBits)
{
    SEI::Tl0DepRepIdxSei* pcTl0DepRepIdxSEI;
    SEI::Tl0DepRepIdxSei::create(pcTl0DepRepIdxSEI);
...
ErrVal LayerEncoder::writeNestingSceneInfoSEIMessage(ExtBinDataAccessorList &rcOutExtBinDataAccessorList, UInt uiTId, UInt uiDId, UInt& ruiBits)
{
    SEI::ScalableNestingSei* pcScalableNestingSei;
    SEI::ScalableNestingSei::create(pcScalableNestingSei);
...
ErrVal LayerEncoder::xWriteNonRequiredSEI(ExtBinDataAccessorList& rcOutExtBinDataAccessorList, SEI::NonRequiredSei* pcNonRequiredSei, UInt& ruiBit)
{
    UInt uiBit = 0;
...
// JVT-V068 HRD {
ErrVal LayerEncoder::xWriteSEI(ExtBinDataAccessorList &rcOutExtBinDataAccessorList, SEI::MessageList& rcSEIMessageList, UInt &ruiBits)
{
    UInt uiBits = 0;
    xInitExtBinDataAccessor(m_cExtBinDataAccessor);

    m_pcNalUnitEncoder->initNalUnit(&m_cExtBinDataAccessor);
    m_pcNalUnitEncoder->write(rcSEIMessageList);
    m_pcNalUnitEncoder->closeNalUnit(uiBits);
    xAppendNewExtBinDataAccessor(rcOutExtBinDataAccessorList, &m_cExtBinDataAccessor);
...
ErrVal LayerEncoder::xWriteNestingSEIforHrd(ExtBinDataAccessorList &rcOutExtBinDataAccessorList, SEI::SEIMessage *pcSEIMessage, UInt uiDependencyId, UInt uiQualityLevel, UInt uiTemporalLevel, UInt &ruiBits)
{
    UInt uiBits = 0;
    xInitExtBinDataAccessor      (m_cExtBinDataAccessor);
...
ErrVal LayerEncoder::xWriteSEIforAVCCompatibleHrd(ExtBinDataAccessorList &rcOutExtBinDataAccessorList, SEI::SEIMessage* pcSEIMessage, UInt &ruiBits)
{
    UInt uiBits = 0;
    xInitExtBinDataAccessor(m_cExtBinDataAccessor);
    m_pcNalUnitEncoder->initNalUnit(&m_cExtBinDataAccessor);
...
ErrVal LayerEncoder::xCalculateTiming(PicOutputDataList&  rcPicOutputDataList, UInt uiFrame)
{
    if(!m_bEnableHrd)
        return Err::m_nOK;
...
//JVT-W049 {
ErrVal LayerEncoder::xWriteRedundantKeyPicSEI (ExtBinDataAccessorList& rcOutExtBinDataAccessorList, UInt &ruiBits)
{
    //===== create message =====
    UInt uiBits = 0;
    xInitExtBinDataAccessor      (m_cExtBinDataAccessor);
    ...
    SEI::RedundantPicSei::create(pcRedundantPicSEI);
    //===== set message =====
...
Void LayerEncoder::xAssignSimplePriorityId(SliceHeader* pcSliceHeader)
{
...
Void LayerEncoder::xPAffDecision(UInt uiFrame)
{
