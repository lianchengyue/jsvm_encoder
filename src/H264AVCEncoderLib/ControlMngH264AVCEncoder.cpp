...
ErrVal ControlMngH264AVCEncoder::initSliceForCoding(const SliceHeader& rcSH)
{
    ...
    for(UInt uiMGSFragments = 0; rcSH.getSPS().getMGSCoeffStop(uiMGSFragments) < 16; uiMGSFragments++)
    {
        ...
        pcCurrentWriter->startSlice(rcSH);
    }
    m_pcMbEncoder->initSlice(rcSH);
    m_pcMbCoder->initSlice(rcSH, m_pcMbSymbolWriteIf, m_pcRateDistortion);
    m_pcMotionEstimation->initSlice(rcSH);
    m_pcSampleWeighting->initSlice(rcSH);
