                                     ...
                                     )
{
    ...
    if (ePicType!=FRAME)
    {
        ...
        if(pcResidualFrameLF)
            pcResidualFrameLF->removeFieldBuffer(ePicType);

        if(pcResidualFrameILPred)
            pcResidualFrameILPred->removeFieldBuffer(ePicType);

        if(pcPredFrame)
            pcPredFrame->removeFieldBuffer(ePicType);

        if(pcBaseIntraRecFrame)
            pcBaseIntraRecFrame->removeFieldBuffer(ePicType);

        if(pcBaseResidualFrame)
            pcBaseResidualFrame->removeFieldBuffer(ePicType);
    }
                                          ...
                                          )
{
    ...
    {
        pcBaseMbDataCtrlField->initSlice(rcSliceHeader, PRE_PROCESS, false, NULL);
    }
    m_pcControlMng->initSliceForCoding(rcSliceHeader);
    UInt uiBitsStart = m_pcMbCoder->getBitCount();
    ...
    for(m_pcMbCoder->bSliceCodedDone = false; uiMbAddress <= uiLastMbAddress;)
    {
        ...
        for(UInt uiMbTest = 0; uiMbTest < 4; uiMbTest++)
        {
            ...
            if(uiFieldMode)
            {
                ...
                if(apcResidualPicLF[uiMbTest]) {
                    acFldMbResLF[uiMbTest].loadBuffer(apcResidualPicLF[uiMbTest]->getFullPelYuvBuffer());
                }

                if(apcResidualPicILPred[uiMbTest]) {
                    acFldMbResILPred[uiMbTest].loadBuffer(apcResidualPicILPred[uiMbTest]->getFullPelYuvBuffer());
                }

                if(apcPredPic[uiMbTest]) {
                    acFldMbPredSignal[uiMbTest].loadBuffer(apcPredPic[uiMbTest]->getFullPelYuvBuffer());
                }
            }
        }
        ...
        for (UInt uiMbIdx = 0; uiMbIdx < 2; uiMbIdx++)
        {
            ...
            m_pcControlMng->initMbForCoding(*pcMbDataAccess,    uiMbY, uiMbX, true, bFieldCodingMode);
            pcMbDataAccess->setMbDataAccessBase(pcMbDataAccessBase);
            ...
            if(bFieldCodingMode)
            {
                ...
                {
                    apcResidualPicLF[uiMbIdx]->getFullPelYuvBuffer()->loadBuffer(&acFldMbResLF[uiMbIdx]);
                }

                if(apcResidualPicILPred[uiMbIdx])
                {
                    apcResidualPicILPred[uiMbIdx]->getFullPelYuvBuffer()->loadBuffer(&acFldMbResILPred [uiMbIdx]);
                }

                if(apcPredPic[uiMbIdx]) {
                    apcPredPic[uiMbIdx]->getFullPelYuvBuffer()->loadBuffer(&acFldMbPredSignal[uiMbIdx]);
                }
            }
    ...
    rcOrgFrame.removeFrameFieldBuffer();
    rcFrame.removeFrameFieldBuffer();
    if(pcResidualFrameLF)
        pcResidualFrameLF->removeFrameFieldBuffer();

    if(pcResidualFrameILPred)
        pcResidualFrameILPred ->removeFrameFieldBuffer();

    if(pcPredFrame)
        pcPredFrame->removeFrameFieldBuffer();

    if(pcBaseIntraRecFrame)
        pcBaseIntraRecFrame->removeFrameFieldBuffer();

    if(pcBaseResidualFrame)
        pcBaseResidualFrame->removeFrameFieldBuffer();
...
//TMM_WP
ErrVal SliceEncoder::xInitDefaultWeights (Double *pdWeights,
