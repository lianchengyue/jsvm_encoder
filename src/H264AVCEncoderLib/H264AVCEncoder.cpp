...
ErrVal H264AVCEncoder::create(H264AVCEncoder*& rpcH264AVCEncoder)
{
...
Frame* H264AVCEncoder::getLowPassRec(UInt uiLayerId, UInt uiLowPassIndex)
{
...
Frame* H264AVCEncoder::getELRefPic(UInt uiLayerId, UInt uiTemporalId, UInt uiFrameIdInTId)
{
    Frame* pcELRefPic = 0;
...
UInt H264AVCEncoder::getMaxSliceSize(UInt uiLayerId, UInt uiAUIndex)
{
...
UInt H264AVCEncoder::getPicCodingType(UInt uiBaseLayerId, UInt uiTemporalId, UInt uiFrmIdInTLayer)
{
...
Bool H264AVCEncoder::hasMGSEnhancementLayer(UInt uiLayerId, UInt& ruiMaxLevelIdc)
{
...
ErrVal H264AVCEncoder::getBaseLayerLevelIdc(UInt uiBaseLayerId, UInt& uiLevelIdc, Bool& bBiPred8x8Disable, Bool& bMCBlks8x8Disable)
{
    m_apcLayerEncoder[uiBaseLayerId]->getBaseLayerLevelIdc(uiLevelIdc, bBiPred8x8Disable, bMCBlks8x8Disable);
    return Err::m_nOK;
}

ErrVal H264AVCEncoder::getBaseLayerStatus(UInt&   ruiBaseLayerId,
                                           UInt    uiLayerId,
                                           PicType ePicType,
                                           UInt       uiTemporalId)
{
    ...
    {
        Bool  bExists   = false;
        m_apcLayerEncoder[ruiBaseLayerId]->getBaseLayerStatus(bExists, ePicType, uiTemporalId);
...
ErrVal H264AVCEncoder::getBaseLayerDataAvlb(Frame*&       pcFrame,
                                             Frame*&       pcResidual,
                                             ...
                                             UInt          uiTemporalId)
{
    ...
    m_apcLayerEncoder[uiBaseLayerId]->getBaseLayerDataAvlb(pcFrame,
                                                            pcResidual,
                                                            pcMbDataCtrl,
                                                            ePicType,
                                                            uiTemporalId);
...
// JVT-AD021 {
ErrVal H264AVCEncoder::getBaseLayerQpPredData (UInt uiBaseLayerId , Double & dQpPredData , Int & iPOC , UInt & uiFrameSizeInMB)
{
    if(uiBaseLayerId >= MAX_LAYERS)
      return(Err::m_nDataNotAvailable);

    m_apcLayerEncoder[uiBaseLayerId]->getCurQpPredData(dQpPredData , iPOC , uiFrameSizeInMB);

    return Err::m_nOK;
}
// JVT-AD021 }

ErrVal H264AVCEncoder::getBaseLayerData(SliceHeader&  rcELSH,
                                         Frame*&       pcFrame,
                                         Frame*&       pcResidual,
                                         MbDataCtrl*&  pcMbDataCtrl,
...
{
    ROF (uiBaseLayerId < MAX_LAYERS);
    m_apcLayerEncoder[uiBaseLayerId]->getBaseLayerData(rcELSH,
                                                              pcFrame,
                                                              pcResidual,
                                                              pcMbDataCtrl,
                                                              bSpatialScalability,
...
ErrVal H264AVCEncoder::getBaseLayerResidual(Frame*&      pcResidual,
                                      UInt            uiBaseLayerId)
{
...
UInt H264AVCEncoder::getNewBits(UInt uiBaseLayerId)
{
...
ErrVal H264AVCEncoder::init (LayerEncoder*     apcLayerEncoder[MAX_LAYERS],
...
ErrVal H264AVCEncoder::xWriteScalableSEI (ExtBinDataAccessor* pcExtBinDataAccessor)
{
    //===== create message =====
    SEI::ScalableSei* pcScalableSEI;
    SEI::ScalableSei::create(pcScalableSEI);
...
//BUG_FIX liuh  ui  }
    ...
    m_apcLayerEncoder[0]->addParameterSetBits(uiBits+4*8);
...
// JVT-S080 LMI {
ErrVal H264AVCEncoder::xWriteScalableSEILayersNotPresent(ExtBinDataAccessor* pcExtBinDataAccessor, UInt uiNumLayers, UInt* uiLayerId)
{
    UInt i, uiBits = 0;
    SEI::MessageList  cSEIMessageList;
    SEI::ScalableSeiLayersNotPresent* pcScalableSeiLayersNotPresent;
    SEI::ScalableSeiLayersNotPresent::create(pcScalableSeiLayersNotPresent);
...
ErrVal H264AVCEncoder::xWriteScalableSEIDependencyChange(ExtBinDataAccessor* pcExtBinDataAccessor, UInt uiNumLayers, UInt* uiLayerId, Bool* pbLayerDependencyInfoPresentFlag,
                          UInt* uiNumDirectDependentLayers, UInt** puiDirectDependentLayerIdDeltaMinus1, UInt* puiLayerDependencyInfoSrcLayerIdDeltaMinus1)
{
    ...
    SEI::ScalableSeiDependencyChange::create(pcScalableSeiDependencyChange);
    ...
    for(uiLayer = 0; uiLayer < uiNumLayers; uiLayer++)
    {
        ...
        if(pcScalableSeiDependencyChange->getLayerDependencyInfoPresentFlag(uiLayer))
        {
            ...
            {
                pcScalableSeiDependencyChange->setDirectDependentLayerIdDeltaMinus1(uiLayer,
                                                                                    uiDirectLayer,
                                                                                    puiDirectDependentLayerIdDeltaMinus1[uiLayer][uiDirectLayer]);
            }
        }
        else
        {
             pcScalableSeiDependencyChange->setLayerDependencyInfoSrcLayerIdDeltaMinus1(uiLayer,
                                                                                        puiLayerDependencyInfoSrcLayerIdDeltaMinus1[uiLayer]);
        }
    }
...
ErrVal H264AVCEncoder::xWriteSubPicSEI (ExtBinDataAccessor* pcExtBinDataAccessor)
{
    SEI::SubPicSei* pcSubPicSEI;
    SEI::SubPicSei::create(pcSubPicSEI);
...
ErrVal H264AVCEncoder::xWriteSubPicSEI (ExtBinDataAccessor* pcExtBinDataAccessor, UInt layer_id)
{
    SEI::SubPicSei* pcSubPicSEI;
    SEI::SubPicSei::create(pcSubPicSEI);
...
ErrVal H264AVCEncoder::xWriteMotionSEI (ExtBinDataAccessor* pcExtBinDataAccessor, UInt sg_id)
{
    //===== create message =====
    SEI::MotionSEI* pcMotionSEI;
    SEI::MotionSEI::create(pcMotionSEI);
                                           ...
                                           Bool &rbMoreSets)
{
    if(m_bVeryFirstCall)
    {
        ...
        {
            xWriteScalableSEI(pcExtBinDataAccessor);
        }
    ...
    if(m_bWrteROISEI)
    {
        LayerParameters& rcLayer = m_pcCodingParameter->getLayerParameters (m_loop_roi_sei/2);
        {
            ...
            if(m_loop_roi_sei%2)
            {
                xWriteMotionSEI(pcExtBinDataAccessor,rcLayer.getSGID()[0]);
                m_loop_roi_sei++;
                return Err::m_nOK;
            }
            else
            {
                xWriteSubPicSEI(pcExtBinDataAccessor, rcLayer.getSLID()[0]);
                m_loop_roi_sei++;
                return Err::m_nOK;
            }
        }
    ...
    if(!m_cUnWrittenSPS.empty())
    {
        ...
        if(m_pcCodingParameter->getNumberOfLayers())
        {
          ...
          m_apcLayerEncoder[uiLayer]->addParameterSetBits(8*(uiSize+4));
        }
...
//JVT-V068 }
    ...
    else
    {
        if(! m_cUnWrittenPPS.empty())
        {
            ...
            m_pcNalUnitEncoder->write(rcPPS);
            m_pcNalUnitEncoder->closeNalUnit(uiBits);
            ...
            if(m_pcCodingParameter->getNumberOfLayers())
            {
                ...
                m_pcParameterSetMng->get(pcSPS, uiSPSId, rcPPS.referencesSubsetSPS());
...
ErrVal H264AVCEncoder::process (ExtBinDataAccessorList&  rcExtBinDataAccessorList,
...
ErrVal H264AVCEncoder::finish (ExtBinDataAccessorList&  rcExtBinDataAccessorList,
                               PicBufferList*  apcPicBufferOutputList,
                               PicBufferList*  apcPicBufferUnusedList,
                               ...
                               Double&  rdHighestLayerOutputRate)
{
    ...
    if(m_pcCodingParameter->getCGSSNRRefinement() == 1)
    {
        for (UInt uiLayer = 1; uiLayer <= m_pcCodingParameter->getNumberOfLayers(); uiLayer++)
        {
            ...
            {
                m_apcLayerEncoder[uiLayer-1]->finish(ruiNumCodedFrames, rdHighestLayerOutputRate, m_aaadFinalFramerate, m_aaadSeqBits);
            }
        }
    }
    else
    {
        ...
        {
            m_apcLayerEncoder[uiLayer]->finish(ruiNumCodedFrames, rdHighestLayerOutputRate, m_aaadFinalFramerate, m_aaadSeqBits);
        }
    }
    printf("\n");
...
ErrVal H264AVCEncoder::xProcessGOP (PicBufferList* apcPicBufferOutputList,
                                    PicBufferList* apcPicBufferUnusedList)
{
    ...
    for (uiLayer = 0; uiLayer <  m_pcCodingParameter->getNumberOfLayers(); uiLayer++)
    {
        ...
        m_apcLayerEncoder[uiLayer]->initGOP(m_cAccessUnitDataList.getAccessUnitData(MSYS_UINT_MAX), m_acOrgPicBufferList[uiLayer]);
    }
    ...
    {                                                                                                                                                                    //JVT-W052
        ...
        {
            SEI::IntegrityCheckSEI::create(m_pcIntegrityCheckSEI); //JVT-W052
        }
        else
        {
        ...
        for(uiLayer   = 0; uiLayer   <  m_pcCodingParameter->getNumberOfLayers(); uiLayer  ++)
        {
            if(m_pcCodingParameter->getNonRequiredEnable())
            {
                if(uiLayer == m_pcCodingParameter->getNumberOfLayers() - 1)
                    m_apcLayerEncoder[uiLayer]->setNonRequiredWrite(2);
                else
                    m_apcLayerEncoder[uiLayer]->setNonRequiredWrite(1);
            }
...
ErrVal H264AVCEncoder::xInitParameterSets()
{
    Bool bOutput    = true;
    UInt uiSPSId    = 0;
    UInt uiPPSId    = 0;
    ...
    for(uiIndex = 0; uiIndex < m_pcCodingParameter->getNumberOfLayers(); uiIndex++)
    {
        ...
        rcLayerParameters.updateWithLevel (m_pcCodingParameter, uiLevelIdc);
        ...
        SequenceParameterSet::create (pcSPS);
        PictureParameterSet ::create (pcPPS);
        ...
        if(rcLayerParameters.getQualityLevelCGSSNR()) // MGS enhancement
        {
            ...
            m_pcParameterSetMng->getActiveSPS(pcQ0SPS, uiCurrDQId - 1);
            pcSPS->copySPSDataForMGSEnhancement(*pcQ0SPS, uiCurrQId); //zhangxd_20101220
        }
...
//TMM_WP
        ...
        m_pcControlMng->initParameterSets (*pcSPS, *pcPPS);
    }
            ...
            for(UInt uiN = 0; uiN < uiNX; uiN++)
            {
                ...
                switch(rcL.getProfileIdc())
                {
                    ...
                    case SCALABLE_HIGH_PROFILE:
                        if(rcL.isIntraOnly())
                            ::printf("Scalable High Intra");
                        else
                            ::printf("Scalable High");
                      break;
                    default:
                      ROT(1);
                }
        ...
        for (UInt uiTemporalId = 0; uiTemporalId < uiTotalTemporalLevel; uiTemporalId++)
        {
            for (UInt uiFGSLevel = 0; uiFGSLevel < uiTotalFGSLevel; uiFGSLevel++)
            {
                ...
                if (m_pcCodingParameter->getEnableNalHRD() || m_pcCodingParameter->getEnableVclHRD())
                {
                    ...
                    pcScheduler->initBuffer(pcVui, uiHrdIdx);
...
ErrVal H264AVCEncoder::writeAVCCompatibleHRDSEI (ExtBinDataAccessor* pcExtBinDataAccessor, SequenceParameterSet& rcSPS)
{
    ...
    if (pcVUI->getVuiParametersPresentFlag() && pcVUI->getNumTemporalLevels() > 1)
    {
        ...
        SEI::AVCCompatibleHRD::create(pcAVCCompHRD, pcVUI);
        cSEIMessageList.push_back(pcAVCCompHRD);
        m_pcNalUnitEncoder->initNalUnit(pcExtBinDataAccessor);
        m_pcNalUnitEncoder->write(cSEIMessageList);
        m_pcNalUnitEncoder->closeNalUnit(uiBitsSei);
    }
    return Err::m_nOK;
}
// JVT-V068 HRD }
...
ErrVal RewriteEncoder::create(RewriteEncoder*& rpcRewriteEncoder)
{
    rpcRewriteEncoder = new RewriteEncoder;
    ROF (rpcRewriteEncoder);
    rpcRewriteEncoder->xCreate();
    return Err::m_nOK;
}


ErrVal RewriteEncoder::destroy()
{
  ROT (m_bInitialized);
...
ErrVal RewriteEncoder::startPicture(const SequenceParameterSet& rcSPS)
{
  ROF(m_bInitialized);
  ROT(m_bPictureInProgress);
...
ErrVal RewriteEncoder::finishPicture(BinDataList& rcBinDataList)
{
  ROF(m_bInitialized);
...
ErrVal RewriteEncoder::rewriteMb(MbDataAccess& rcMbDataAccessSource, Bool bSendEOS)
{
  ROF(m_bInitialized);
  ROF(m_bPictureInProgress);
...
ErrVal RewriteEncoder::xStartSlice(MbDataAccess& rcMbDataAccessSource)
{
...
ErrVal RewriteEncoder::xFinishSlice()
{
    xCloseNALUnit();
...
ErrVal RewriteEncoder::xInitNALUnit()
{
    ...
    m_pcNalUnitEncoder->initNalUnit(m_pcBinDataAccessor);
    return Err::m_nOK;
}


ErrVal RewriteEncoder::xCloseNALUnit()
{
    UInt uiBits = 0;
    m_pcNalUnitEncoder->closeNalUnit(uiBits);
...
Bool RewriteEncoder::xIsRewritten(const Void* pParameterSet)
{
...
ErrVal RewriteEncoder::xRewriteSPS(const SequenceParameterSet& rcSPS)
{
...
ErrVal RewriteEncoder::xRewritePPS(const PictureParameterSet& rcPPS)
{
...
ErrVal RewriteEncoder::xInitMb(MbDataAccess*& rpcMbDataAccessRewrite, MbDataAccess& rcMbDataAccessSource)
{
  ...
  m_pcMbDataCtrl->initMb(rpcMbDataAccessRewrite, uiMbY, uiMbX);
...
ErrVal RewriteEncoder::xAdjustMb(MbDataAccess& rcMbDataAccessRewrite, Bool bBaseLayer)
{
  ...
  if(!rcMbDataAccessRewrite.getMbData().isIntra())
  {
    ...
    for(uiList = 0; uiList < 2; uiList++)
    {
      ...
      {
          m_pcMotionVectorCalculation->calcMvMb (rcMbDataAccessRewrite, 0);
...
ErrVal RewriteEncoder::xEncodeMb(MbDataAccess& rcMbDataAccessRewrite, Bool bLastMbInSlice, Bool bSendEOS)
{
    //===== encode =====
    m_pcMbCoder->encode(rcMbDataAccessRewrite, NULL, bLastMbInSlice, bSendEOS);
    return Err::m_nOK;
}
