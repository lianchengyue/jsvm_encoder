...
ErrVal SliceHeader::init(const SequenceParameterSet& rcSPS, const PictureParameterSet& rcPPS)
{
    SliceHeaderSyntax::init(rcSPS, rcPPS);
    m_eErrorConcealMode   = EC_NONE;
    m_bTrueSlice          = true;
    m_uiNumMbsInSlice     = 0;
    m_uiLastMbInSlice     = 0;
...
Int SliceHeader::getPoc(PicType ePicType) const
{
    return (ePicType == FRAME ? gMin(m_iTopFieldPoc, m_iBotFieldPoc) : ePicType == BOT_FIELD ? m_iBotFieldPoc : m_iTopFieldPoc);
}

Void SliceHeader::setSCoeffResidualPredFlag(ResizeParameters* pcResizeParameters)
{
    m_bSCoeffResidualPred = false;
...
ErrVal SliceHeader::sliceHeaderBackup(SliceHeader* pcSH)
{
...
ErrVal SliceHeader::FMOUninit()
{
    m_cFMO.finit();
    return Err::m_nOK;
}

Int SliceHeader::getNumMbInSlice()
{
...
Int SliceHeader::getDistScaleFactor(PicType eMbPicType,
                                    SChar   sL0RefIdx,
