...
ErrVal CoeffLevelPred::ScaleCoeffLevels(TCoeff* piCoeff,
                                        TCoeff* piRefCoeff,
                                        UInt uiQp,
                                        UInt uiRefQp,
                                        UInt uiNumCoeffs)
...
Void Transform::xQuantDequantNonUniformLuma (TCoeff* piQCoeff,
                                             TCoeff* piCoeff,
                                             const QpParameter& rcQp,
                                             const UChar* pucScale,
                                             UInt& ruiDcAbs,  //DC分量绝对值
                                             UInt& ruiAcAbs)  //AC分量绝对值
{
    Int   iLevel = piCoeff[0];
    UInt  uiSign = ((UInt)iLevel)>>31;
    Int   iAdd   = (1 << 3) >> rcQp.per();

    ///g_aaiQuantCoef[6][16]:  4x4x4表
    //量化
...
ErrVal Transform::predictScaledACCoeffs(UInt uiComp, TCoeff *piCoeff, TCoeff *piRef, UInt uiRefQp, const UChar* pucScale)
{
    // DECLARATIONS
...
ErrVal Transform::predictScaledChromaCoeffs(TCoeff *piCoeff, TCoeff *piRef, UInt uiRefCbQp, UInt uiRefCrQp, const UChar* pucScaleCb, const UChar* pucScaleCr)
{
    // DECLARATIONS
...
ErrVal Transform::addPrediction8x8Blk(TCoeff* piCoeff,
                                      TCoeff* piRefCoeff,
                                      UInt uiQp,
                                      UInt uiRefQp,
                                      Bool& bCoded)
...
ErrVal Transform::addPredictionChromaBlocks(TCoeff* piCoeff,
                                            TCoeff* piRef,
                                            UInt uiQp,
                                            UInt uiRefQp,
                                            Bool& bDCflag,
...
ErrVal Transform::invTransformChromaBlocks(Pel* puc,
                                           Int iStride,
                                           TCoeff* piCoeff)
{
                                  ...
                                  UInt&         ruiAbsSum)
{
    ...
    xForTransform4x4Blk(pOrg,
                        pRec,
                        iStride,
                        aiTemp);
    xQuantDequantUniform4x4(piCoeff,
                            aiTemp,
                            m_cLumaQp,
                            pucScale,
                            ruiAbsSum);
...
ErrVal Transform::transform8x8BlkCGS(YuvMbBuffer* pcOrgData,
                                     YuvMbBuffer* pcPelData,
                                     TCoeff*      piCoeff,
                                     TCoeff*      piCoeffBase,
                                     const UChar* pucScale,
...
Void Transform::xForTransform4x4Blk(XPel* pucOrg,
                                    XPel* pucRec,
                                    Int iStride,
                                    TCoeff* piPredCoeff)
{
...
//16x16DCT变换
ErrVal Transform::transformMb16x16(YuvMbBuffer* pcOrgData,
                                   YuvMbBuffer* pcPelData,
                                   TCoeff* piCoeff,
                                   const UChar* pucScale,
                                   UInt& ruiDcAbs,
                                   UInt& ruiAcAbs)
{
    ...
    for(n = 0; n < 16; n ++)
    {
        xQuantDequantNonUniformLuma(&piCoeff[n<<4],
                                    &aiCoeff[n<<4],
                                    m_cLumaQp,
                                    pucScale,
                                    ruiDcAbs,
    ...
    for(n = 0; n < 16; n += 4)
    {
        for(x = 0; x < 4; x++)
        {
            UInt uiBlk = x+n;
            Int iOffsetBlk = iOffset + (x << 2);
            xInvTransform4x4Blk(pucRec + iOffsetBlk,
                                iStride,
                                &aiCoeff[uiBlk<<4]);
        }
        iOffset += iStride << 2;
...
ErrVal Transform::invTransformChromaBlocks(XPel* puc,
                                           Int iStride,
                                           TCoeff* piCoeff)
{
    xInvTransform4x4Blk(puc,     iStride, piCoeff + 0x00);
...
ErrVal Transform::invTransform4x4Blk(XPel* puc,
                                     Int iStride,
                                     TCoeff* piCoeff)
{
    xInvTransform4x4Blk(puc, iStride, piCoeff);
...
ErrVal Transform::transform8x8Blk(YuvMbBuffer* pcOrgData,
                                  YuvMbBuffer* pcPelData,
                                  TCoeff*         piCoeff,
                                  const UChar*    pucScale,
                                  UInt&           ruiAbsSum)
{
    ...
    xForTransform8x8Blk (pOrg,
                         pRec,
                         iStride,
                         aiTemp);
    //量化:quantity
    //8X8 Blocl的量化与反量化
    xQuantDequantUniform8x8 (piCoeff,
                             aiTemp,
                             m_cLumaQp,
                             pucScale,
                             ruiAbsSum);
...
ErrVal Transform::invTransform8x8Blk(XPel*  puc,
                                     Int    iStride,
                                     TCoeff*  piCoeff)
{
    Int aai[8][8];
...
Void Transform::x4x4DequantChroma(TCoeff*  piQCoeff,
                                  TCoeff*  piCoeff,
                                  const QpParameter&  rcQp,
                                  const UChar*        pucScale)
{
...
Void Transform::xForTransform8x8Blk(XPel* pucOrg,
                                    XPel* pucRec,
                                    Int iStride,
                                    TCoeff* piPredCoeff)
{
...
Void Transform::xQuantDequantUniform8x8 (TCoeff*  piQCoeff,
                                         TCoeff*  piCoeff,
                                         const QpParameter&  rcQp,
                                         const UChar*  pucScale,
                                         UInt&         ruiAbsSum)
