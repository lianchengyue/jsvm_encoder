...
{
    pcWriteIf->writeFlag(m_bBitstreamRestrictionFlag,              "VUI: bitstream_restriction_flag");
    ROFRS(m_bBitstreamRestrictionFlag, Err::m_nOK);

    pcWriteIf->writeFlag(getMotionVectorsOverPicBoundariesFlag(),  "VUI: motion_vectors_over_pic_boundaries_flag");
    pcWriteIf->writeUvlc(getMaxBytesPerPicDenom(),                 "VUI: max_bytes_per_pic_denom");
    pcWriteIf->writeUvlc(getMaxBitsPerMbDenom(),                   "VUI: max_bits_per_mb_denom");
    pcWriteIf->writeUvlc(getLog2MaxMvLengthHorizontal(),           "VUI: log2_max_mv_length_horizontal");
    pcWriteIf->writeUvlc(getLog2MaxMvLengthVertical(),             "VUI: log2_max_mv_length_vertical");
    pcWriteIf->writeUvlc(getMaxDecFrameReordering(),               "VUI: max_dec_frame_reordering");
    pcWriteIf->writeUvlc(getMaxDecFrameBuffering(),                "VUI: max_dec_frame_buffering");
    return Err::m_nOK;
}
...
Bool VUI::BitstreamRestriction::isSame(const VUI::BitstreamRestriction& rcBitstreamRestriction)  const
{
...
Bool VUI::AspectRatioInfo::isSame(const VUI::AspectRatioInfo& rcAspectRatioInfo)  const
{
...
ErrVal VUI::AspectRatioInfo::copy(const VUI::AspectRatioInfo& rcAspectRatioInfo)
{
...
Bool VUI::VideoSignalType::isSame(const VUI::VideoSignalType& rcVideoSignalType) const
{
...
ErrVal VUI::VideoSignalType::copy(const VUI::VideoSignalType& rcVideoSignalType)
{
...
Bool VUI::ChromaLocationInfo::isSame(const VUI::ChromaLocationInfo& rcChromaLocationInfo)  const
{
...
ErrVal VUI::ChromaLocationInfo::copy(const VUI::ChromaLocationInfo& rcChromaLocationInfo)
{
...
Bool VUI::TimingInfo::isSame(const VUI::TimingInfo& rcTimingInfo) const
{
...
ErrVal VUI::TimingInfo::copy(const VUI::TimingInfo& rcTimingInfo)
{
...
{
    pcWriteIf->writeFlag(m_bAspectRatioInfoPresentFlag,    "VUI: aspect_ratio_info_present_flag");
    ROFRS(m_bAspectRatioInfoPresentFlag, Err::m_nOK);

    pcWriteIf->writeCode(m_uiAspectRatioIdc, 8,            "VUI: aspect_ratio_idc");

    if(0xFF == m_uiAspectRatioIdc) //Extendet_SAR
    {
        pcWriteIf->writeCode(m_uiSarWith, 16,                "VUI: sar_width");
        pcWriteIf->writeCode(m_uiSarHeight, 16,              "VUI: sar_height");
    }
    return Err::m_nOK;
}
...
{
    pcWriteIf->writeFlag(m_bVideoSignalTypePresentFlag,    "VUI: video_signal_type_present_flag");
    ROFRS(m_bVideoSignalTypePresentFlag, Err::m_nOK);

    pcWriteIf->writeCode(m_uiVideoFormat, 3,               "VUI: video_format");
    pcWriteIf->writeFlag(m_bVideoFullRangeFlag,            "VUI: video_full_range_flag");
    pcWriteIf->writeFlag(m_bColourDescriptionPresentFlag,  "VUI: colour_description_present_flag");
    if(m_bColourDescriptionPresentFlag)
    {
        pcWriteIf->writeCode(m_uiColourPrimaries, 8,         "VUI: colour_primaries");
        pcWriteIf->writeCode(m_uiTransferCharacteristics, 8, "VUI: transfer_characteristics");
        pcWriteIf->writeCode(m_uiMatrixCoefficients, 8,      "VUI: matrix_coefficients");
    }
    return Err::m_nOK;
}
...
{
    pcWriteIf->writeFlag(m_bChromaLocationInfoPresentFlag, "VUI: chroma_location_info_present_flag");
    ROFRS(m_bChromaLocationInfoPresentFlag, Err::m_nOK);

    pcWriteIf->writeUvlc(m_uiChromaLocationFrame,          "VUI: chroma_location_frame");
    pcWriteIf->writeUvlc(m_uiChromaLocationField,          "VUI: chroma_location_field");
    return Err::m_nOK;
}

ErrVal VUI::TimingInfo::write(HeaderSymbolWriteIf* pcWriteIf) const
{
    pcWriteIf->writeFlag(m_bTimingInfoPresentFlag,         "VUI: timing_info_present_flag");
    ROFRS(m_bTimingInfoPresentFlag, Err::m_nOK);

    pcWriteIf->writeCode(m_uiNumUnitsInTick, 32,           "VUI: num_units_in_tick");
    pcWriteIf->writeCode(m_uiTimeScale, 32,                "VUI: time_scale");
    pcWriteIf->writeFlag(m_bFixedFrameRateFlag,            "VUI: fixed_frame_rate_flag");
    return Err::m_nOK;
}
...
Bool VUI::isSameExceptHRDParametersAndSVCExt(const VUI& rcVUI) const
{
...
ErrVal VUI::copyExceptHRDParametersAndSVCExt(const VUI& rcVUI)
{
    ...
    m_cAspectRatioInfo     .copy(rcVUI.m_cAspectRatioInfo    );
    m_cVideoSignalType     .copy(rcVUI.m_cVideoSignalType    );
    m_cChromaLocationInfo  .copy(rcVUI.m_cChromaLocationInfo );
    m_cBitstreamRestriction.copy(rcVUI.m_cBitstreamRestriction);
    return Err::m_nOK;
}
...
{
    pcWriteIf->writeCode(m_uiDependencyID, 3,  "VUI: dependency_id");
    pcWriteIf->writeCode(m_uiQualityId,    4,  "VUI: quality_level");
    pcWriteIf->writeCode(m_uiTemporalId,   3,  "VUI: temporal_level");
    return Err::m_nOK;
}

ErrVal VUI::write(HeaderSymbolWriteIf* pcWriteIf) const
{
    pcWriteIf->writeFlag(m_bVuiParametersPresentFlag,         "SPS: vui_parameters_present_flag");

    if (!m_bVuiParametersPresentFlag) return Err::m_nOK;

    m_cAspectRatioInfo.write(pcWriteIf);

    pcWriteIf->writeFlag(getOverscanInfoPresentFlag(),       "VUI: overscan_info_present_flag");
    if(getOverscanInfoPresentFlag())
    {
        pcWriteIf->writeFlag(getOverscanAppropriateFlag(),   "VUI: overscan_appropriate_flag");
    }

    m_cVideoSignalType.write(pcWriteIf);
    m_cChromaLocationInfo.write(pcWriteIf);

    if(m_eProfileIdc == SCALABLE_BASELINE_PROFILE || m_eProfileIdc == SCALABLE_HIGH_PROFILE)
    {
        pcWriteIf->writeFlag(false,  "VUI: timing_info_present_flag"  );
        pcWriteIf->writeFlag(false,  "HRD: hdr_parameters_present_flag");
        pcWriteIf->writeFlag(false,  "HRD: hdr_parameters_present_flag");
        pcWriteIf->writeFlag(false,  "VUI: pic_struct_present_flag"   );
    }
    else
    {
        ROT(m_uiDefaultIdx == MSYS_UINT_MAX);
        m_acTimingInfo.get(m_uiDefaultIdx).write(pcWriteIf);
        m_acNalHrd.get(m_uiDefaultIdx).write(pcWriteIf);
        m_acVclHrd.get(m_uiDefaultIdx).write(pcWriteIf);
        if(m_acNalHrd.get(m_uiDefaultIdx).getHrdParametersPresentFlag() || m_acVclHrd.get(m_uiDefaultIdx).getHrdParametersPresentFlag())
        {
            pcWriteIf->writeFlag(m_abLowDelayHrdFlag.get(m_uiDefaultIdx),         "VUI: low_delay_hrd_flag");
        }
        pcWriteIf->writeFlag(m_abPicStructPresentFlag.get(m_uiDefaultIdx),        "VUI: pic_struct_present_flag");
    }

    m_cBitstreamRestriction.write(pcWriteIf);
...
{
    ROF(getNumLayers());
    pcWriteIf->writeUvlc(getNumLayers() - 1,            "SPS: num_layers_minus1");

    for(UInt uiLayer = 0; uiLayer < getNumLayers(); uiLayer++)
    {
        m_acLayerInfo.get(uiLayer).write(pcWriteIf);
        m_acTimingInfo.get(uiLayer).write(pcWriteIf);
        m_acNalHrd.get(uiLayer).write(pcWriteIf);
        m_acVclHrd.get(uiLayer).write(pcWriteIf);
        if(m_acNalHrd.get(uiLayer).getHrdParametersPresentFlag() || m_acVclHrd.get(uiLayer).getHrdParametersPresentFlag())
        {
            pcWriteIf->writeFlag(m_abLowDelayHrdFlag.get(uiLayer),           "VUI: low_delay_hrd_flag");
        }
        pcWriteIf->writeFlag(m_abPicStructPresentFlag.get(uiLayer),        "VUI: pic_struct_present_flag");
    }
...
{
    UInt uiNumLayers = 0;
    pcReadIf->getUvlc(uiNumLayers,            "SPS: num_layers_minus1");
    uiNumLayers++;
    ...
    {
        m_acLayerInfo.get(uiLayer).read(pcReadIf);
        m_acTimingInfo.get(uiLayer).read(pcReadIf);
        m_acNalHrd.get(uiLayer).read(pcReadIf);
        m_acVclHrd.get(uiLayer).read(pcReadIf);
        if(m_acNalHrd.get(uiLayer).getHrdParametersPresentFlag() || m_acVclHrd.get(uiLayer).getHrdParametersPresentFlag())
        {
            pcReadIf->getFlag(m_abLowDelayHrdFlag.get(uiLayer),           "VUI: low_delay_hrd_flag");
        }
        pcReadIf->getFlag(m_abPicStructPresentFlag.get(uiLayer),          "VUI: pic_struct_present_flag");
    }
