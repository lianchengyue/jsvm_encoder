...
{
    MotionVectorCalculation::uninit();
...
//预测Luma
Void MotionCompensation::xPredLuma(YuvMbBuffer* pcRecBuffer,
                                   Int iSizeX,
                                   Int iSizeY,
                                   MC8x8& rcMc8x8D)
{
    ...
    for(Int n = 0; n < 2; n++)
    {
        ...
        if(NULL != pcRefBuffer)
        {
            ...
            m_pcQuarterPelFilter->predBlk(apcTarBuffer[n],
                                          pcRefBuffer,
                                          rcMc8x8D.m_cIdx,
                                          cMv,
                                          iSizeY,
...
//预测Luma
Void MotionCompensation::xPredLuma(YuvMbBuffer* apcTarBuffer[2],
                                   Int iSizeX,
                                   Int iSizeY,
                                   MC8x8& rcMc8x8D,
                                   SParIdx4x4 eSParIdx)
...
//xPredChroma的具体实现
__inline Void MotionCompensation::xPredChromaPel(XPel* pucDest,
                                                 Int iDestStride,
                                                 XPel* pucSrc,
                                                 Int iSrcStride,
                                                 Mv cMv,
...
__inline Void MotionCompensation::xPredChroma(YuvMbBuffer* pcDesBuffer,
                                              YuvPicBuffer* pcSrcBuffer,
                                              LumaIdx cIdx,
                                              Mv cMv,
                                              Int iSizeY,
                                              Int iSizeX)
{
    ...
    xPredChromaPel(pcDesBuffer->getUBlk(cIdx),
                   iDesStride,
                   pcSrcBuffer->getUBlk(cIdx)+ iOffset,
                   iSrcStride,
                   cMv,
                   iSizeY,
                   iSizeX);

    xPredChromaPel(pcDesBuffer->getVBlk(cIdx),
                   iDesStride,
                   pcSrcBuffer->getVBlk(cIdx)+ iOffset,
                   iSrcStride,
                   cMv,
                   iSizeY,
                   iSizeX);
...
Void MotionCompensation::xPredChroma(YuvMbBuffer* pcRecBuffer,
                                     Int iSizeX,
                                     Int iSizeY,
                                     MC8x8& rcMc8x8D)
{
...
Void MotionCompensation::xPredChroma(YuvMbBuffer* apcTarBuffer[2],
                                     Int iSizeX,
                                     Int iSizeY,
                                     MC8x8& rcMc8x8D,
                                     SParIdx4x4 eSParIdx)
    ...
    case MODE_SKIP:
      {
        ...
        {
            B8x8Idx c8x8Idx;
            updateDirectBlock(rcMbDataAccess, pcMCFrame, pcPrdFrame, eListPrd, iRefIdx, c8x8Idx); c8x8Idx++;
            updateDirectBlock(rcMbDataAccess, pcMCFrame, pcPrdFrame, eListPrd, iRefIdx, c8x8Idx); c8x8Idx++;
            updateDirectBlock(rcMbDataAccess, pcMCFrame, pcPrdFrame, eListPrd, iRefIdx, c8x8Idx); c8x8Idx++;
            updateDirectBlock(rcMbDataAccess, pcMCFrame, pcPrdFrame, eListPrd, iRefIdx, c8x8Idx); ;
        }
        else
        {
            MC8x8 cMC8x8D(B_8x8_0);
...
Void MotionCompensation::xUpdateBlk(Frame* pcPrdFrame,
                                    Int iSizeX,
                                    Int iSizeY,
                                    MC8x8& rcMc8x8D)
{
...
Void MotionCompensation::updateBlkAdapt(YuvPicBuffer* pcSrcBuffer,
                                        YuvPicBuffer* pcDesBuffer,
                                        LumaIdx cIdx,
                                        Mv cMv,
                                        Int iSizeY,
                                        Int iSizeX,
...
#define BUF_W   19
#define BUF_H   19
Void MotionCompensation::xUpdAdapt(XPel* pucDest,
                                   XPel* pucSrc,
                                   Int iDestStride,
                                   Int iSrcStride,
                                   Int iDx,
...
__inline Void MotionCompensation::xUpdateChroma(YuvPicBuffer* pcSrcBuffer,
                                                YuvPicBuffer* pcDesBuffer,
                                                LumaIdx cIdx,
                                                Mv cMv,
                                                Int iSizeY,
...
__inline Void MotionCompensation::xUpdateChromaPel(XPel* pucDest,
                                                   Int iDestStride,
                                                   XPel* pucSrc,
                                                   Int iSrcStride,
                                                   Mv cMv,
                                                   ...
                                                   Frame*        pcBaseLayerRec)
{
    ...
    {
        copyMbBuffer(&cBaseMbRec, pcRecBuffer, 0, 0, iCX, iCY);
    }
    if(rcMbData.isBaseIntra(1, 0) && iCX < 16)
    {
        copyMbBuffer(&cBaseMbRec, pcRecBuffer, iCX, 0, 16, iCY);
    }
    if(rcMbData.isBaseIntra(0, 1) && iCY < 16)
    {
        copyMbBuffer(&cBaseMbRec, pcRecBuffer, 0, iCY, iCX, 16);
    }
    if(rcMbData.isBaseIntra(1, 1) && iCX < 16 && iCY < 16)
    {
        copyMbBuffer(&cBaseMbRec, pcRecBuffer, iCX, iCY, 16, 16);
    }
    return Err::m_nOK;
}


ErrVal MotionCompensation::updateMbBLSkipResidual(MbDataAccess& rcMbDataAccess,
                                            YuvMbBuffer&  rcMbResBuffer)
{
    ...
    if(rcMbData.isBaseIntra(0, 0))
    {
        clearMbBuffer(rcMbResBuffer, 0, 0, iCX, iCY);
    }
    if(rcMbData.isBaseIntra(1, 0) && iCX < 16)
    {
       clearMbBuffer(rcMbResBuffer, iCX, 0, 16, iCY);
    }
    if(rcMbData.isBaseIntra(0, 1) && iCY < 16)
    {
       clearMbBuffer(rcMbResBuffer, 0, iCY, iCX, 16);
    }
    if(rcMbData.isBaseIntra(1, 1) && iCX < 16 && iCY < 16)
    {
       clearMbBuffer(rcMbResBuffer, iCX, iCY, 16, 16);
    }
    return Err::m_nOK;
}
...
ErrVal MotionCompensation::clearMbBuffer(YuvMbBuffer& rcMbBuffer, Int sX, Int sY, Int eX, Int eY)
{
...
ErrVal MotionCompensation::compensateMb(MbDataAccess&    rcMbDataAccess,
                                        ...
                                        Bool             bCalcMv)
{
    MbMode eMbMode  = rcMbDataAccess.getMbData().getMbMode();  //m_eMbMode
        ...
        case MODE_SKIP:
        {
            if(rcMbDataAccess.getSH().isBSlice())
            {
                if(bCalcMv)
                {
                    ...
                    {
                        rcMbDataAccess.setSVCDirectModeMvAndRef(*pcL0, *pcL1);
                    }
                }

                B8x8Idx c8x8Idx;
                compensateDirectBlock(rcMbDataAccess, pcRecBuffer, c8x8Idx, rcRefFrameList0, rcRefFrameList1); c8x8Idx++;
                compensateDirectBlock(rcMbDataAccess, pcRecBuffer, c8x8Idx, rcRefFrameList0, rcRefFrameList1); c8x8Idx++;
                compensateDirectBlock(rcMbDataAccess, pcRecBuffer, c8x8Idx, rcRefFrameList0, rcRefFrameList1); c8x8Idx++;
                compensateDirectBlock(rcMbDataAccess, pcRecBuffer, c8x8Idx, rcRefFrameList0, rcRefFrameList1); ;
            }
            else
            {
//    TMM_EC {{
