...
ErrVal MotionUpsampling::resample(Int iMbXCurr, Int iMbYCurr)
{
    xInitMb(iMbXCurr, iMbYCurr);
    xSetPartIdcArray();
    ...
    if(m_bInCropWindow && ! m_bIntraBL)
    {
        ...
        {
            xDeriveBlockModeAndUpdateMv(iB8x8Idx);
        }
        xDeriveMbMode();
        xDeriveFwdBwd();
        xSetInterIntraIdc();
...
ErrVal MotionUpsampling::xInitMb(Int iMbXCurr, Int iMbYCurr)
{
    m_iMbXCurr              = iMbXCurr;
    m_iMbYCurr              = iMbYCurr;
...
Bool MotionUpsampling::xIsInCropWindow()
{
...
ErrVal MotionUpsampling::xGetRefLayerMb(Int   iXInsideCurrMb,
                                        Int   iYInsideCurrMb,
                                        Int&  riBaseMbIdx,
                                        Int&  riXInsideBaseMb,
                                        Int&  riYInsideBaseMb)
...
ErrVal MotionUpsampling::xGetRefLayerPartIdc(Int  iXInsideCurrMb,
                                             Int  iYInsideCurrMb,
                                             Int& riPartIdc)
{
...
ErrVal MotionUpsampling::xSetPartIdcArray()
{
    ...
    //===== determine all 16 initial partition indexes =====
    {
        ...
        for(Int iY = 0; iY < 4; iY++)
        {
            ...
            {
                xGetRefLayerPartIdc((iX << 2) + 1, (iY << 2) + 1, m_aaiPartIdc[iX][iY]);
    ...
    //===== replace values of "-1" on a 4x4 block basis =====
    {
        for(Int iYP = 0; iYP < 2; iYP++)
        {
            ...
            {
                Bool aabProcI4x4Blk[2][2] = { {false, false},
                                              {false, false} };
    ...
    {
        Bool aabProcI8x8Blk[2][2] = { {false, false},
                                      {false, false} };
...
ErrVal MotionUpsampling::xGetInitialBaseRefIdxAndMv(Int     i4x4BlkX,
                                                    Int     i4x4BlkY,
                                                    ListIdx eListIdx,
                                                    Int     iPartIdc,
                                                    Int&    riRefIdx,
...
Int MotionUpsampling::xGetMinRefIdx(Int iRefIdxA, Int iRefIdxB)
{
...
ErrVal MotionUpsampling::xGetRefIdxAndInitialMvPred(ListIdx eListIdx)
{
    //===== get initial predictors for reference indices and motion vectors =====
    {
        for(Int i4x4BlkY = 0; i4x4BlkY < 4; i4x4BlkY++)
        {
            ...
            {
                xGetInitialBaseRefIdxAndMv(i4x4BlkX,
                                           i4x4BlkY,
                                           eListIdx,
                                           m_aaiPartIdc[i4x4BlkX][i4x4BlkY],
                                           m_aaiRefIdxTemp[i4x4BlkX][i4x4BlkY],
                                           m_aaacMv[eListIdx][i4x4BlkX][i4x4BlkY]);
            }
        }
    }
...
Int MotionUpsampling::xMvDiff(const Mv& rcMvA, const Mv& rcMbB)
{
...
ErrVal MotionUpsampling::xDeriveBlockModeAndUpdateMv(Int i8x8BlkIdx)
{
...
Bool MotionUpsampling::x8x8BlocksHaveSameMotion(ListIdx eListIdx, Int i8x8IdxA, Int i8x8IdxB)
{
...
ErrVal MotionUpsampling::xDeriveMbMode()
{
...
ErrVal MotionUpsampling::xDeriveFwdBwd()
{
    m_uiFwdBwd = 0;
    for(Int n = 3; n >= 0; n--)
    {
...
ErrVal MotionUpsampling::xSetInterIntraIdc()
{
    ...
    for(Int iY = 0; iY < 2; iY++)
    {
        for(Int iX = 0; iX < 2; iX++)
        {
            ...
            Int iYPos     = iY * 15;
            Int iPartIdc  = -2;
            xGetRefLayerPartIdc(iXPos, iYPos, iPartIdc);
...
ErrVal MotionUpsampling::xSetResPredSafeFlag()
{
    ...
    for(Int iY = 0; iY < 4 && m_bResPredSafe; iY++)
    {
        for(Int iX = 0; iX < 4 && m_bResPredSafe; iX++)
        {
            ...
            for(k = 0; k < 4; k++)
            {
                xGetRefLayerPartIdc(iXPos0+3*(k&1), iYPos0+3*(k>>1), aiPartIdc[k]);
...
ErrVal MbDataCtrl::copyMotion(MbDataCtrl& rcMbDataCtrl, PicType ePicType)
{
    ...
    for(Int y = 0; y < (Int)uiNumLines; y++, pcMbDataDes += uiStride, pcMbDataSrc += uiStride)
    {
        ...
        {
            pcMbDataDes[x].copyMotion(pcMbDataSrc[x], m_uiSliceId);
        }
    }
...
ErrVal MbDataCtrl::upsampleMotion(SliceHeader*      pcSliceHeader,
                                  ...
                                  Int               iMvThreshold)
{
    ...
    for(Int iMbY = 0; iMbY < iMbYMax; iMbY++)
    {
        ...
        {
            cMotionUpsampling.resample(iMbX, iMbY);
        }
    }
    return Err::m_nOK;
}
...
ErrVal MbDataCtrl::initSliceLF(SliceHeader& rcSH, const MbStatus* apcMbStatus)
{
    initSlice(rcSH, POST_PROCESS, false, NULL);
...
ErrVal MbDataCtrl::getBoundaryMask(Int iMbY,
                                   Int iMbX,
                                   Bool& rbIntra,
                                   UInt& ruiMask,
                                   UInt uiCurrentSliceID) const
{
    ruiMask = 0;
...
ErrVal MbDataCtrl::getBoundaryMask_MbAff(Int iMbY, Int iMbX, Bool& rbIntra, UInt& ruiMask, UInt uiCurrentSliceID) const
{
