...
ErrVal YuvPicBuffer::init(XPel*& rpucYuvBuffer)
{
    ...
    //
    UInt uiSize;
    m_rcYuvBufferCtrl.getChromaSize(uiSize);
...
ErrVal YuvPicBuffer::loadFromPicBuffer(PicBuffer* pcPicBuffer)
{
    ROF(pcPicBuffer);
...
ErrVal YuvPicBuffer::clearCurrMb()
{
...
ErrVal YuvPicBuffer::interpolatedPicBuffer(PicBuffer* pcPicBuffer, Bool bBotField)
{
    ROF(pcPicBuffer);
...
ErrVal YuvPicBuffer::inversepredictionSlices(YuvPicBuffer*  pcSrcYuvPicBuffer,
...
ErrVal YuvPicBuffer::copyMb(YuvPicBuffer* pcSrcYuvPicBuffer,
                            UInt uiMbY,
                            UInt uiMbX)
{
    pcSrcYuvPicBuffer->m_rcYuvBufferCtrl.initMb(uiMbY,uiMbX,false);
...
ErrVal YuvPicBuffer::dumpHPS(FILE* pFile,
...
Bool YuvPicBuffer::isCurr4x4BlkNotZero(LumaIdx c4x4Idx)
{
    Int   iStride = getLStride  ();
...
Bool YuvPicBuffer::isCurr8x8BlkNotZero(B8x8Idx c8x8Idx)
{
    Int iStride = getLStride();
...
Void YuvPicBuffer::xCopyFillPlaneMargin(XPel *pucSrc,
                                        XPel *pucDest,
                                        Int iHeight,
                                        Int iWidth,
                                        Int iStride,
...
Void YuvPicBuffer::xCopyPlane(XPel *pucSrc,
                              XPel *pucDest,
                              Int iHeight,
                              Int iWidth,
                              Int iStride)
...
ErrVal YuvPicBuffer::loadBuffer(YuvPicBuffer *pcSrcYuvPicBuffer)
{
    ...
    xCopyPlane(iSrcOffset + pcSrcYuvPicBuffer->getMbLumAddr(),
               getMbLumAddr(),
               getLHeight(),
               getLWidth(),
               getLStride());
    iSrcOffset >>= 1;
    xCopyPlane(iSrcOffset + pcSrcYuvPicBuffer->getMbCbAddr(),
               getMbCbAddr(),
               getCHeight(),
               getCWidth(),
               getCStride());
    xCopyPlane(iSrcOffset + pcSrcYuvPicBuffer->getMbCrAddr(),
               getMbCrAddr(),
               getCHeight(),
               getCWidth(),
               getCStride());
...
ErrVal YuvPicBuffer::loadBufferAndFillMargin(YuvPicBuffer *pcSrcYuvPicBuffer)
{
    ...
    xCopyFillPlaneMargin(iSrcOffset + pcSrcYuvPicBuffer->getMbLumAddr(),
                         getMbLumAddr(),
                         getLHeight(),
                         getLWidth(),
                         getLStride(),
                         ...
                         getLYMargin());
    iSrcOffset >>= 1;
    xCopyFillPlaneMargin(iSrcOffset + pcSrcYuvPicBuffer->getMbCbAddr(),
                         getMbCbAddr(),
                         getCHeight(),
                         getCWidth(),
                         getCStride(),
                         getCXMargin(),
                         getCYMargin());
    xCopyFillPlaneMargin(iSrcOffset + pcSrcYuvPicBuffer->getMbCrAddr(),
                         getMbCrAddr(),
                         getCHeight(),
                         getCWidth(),
                         getCStride(),
...
ErrVal YuvPicBuffer::fillMargin()
{
    ...
    xFillPlaneMargin(getMbLumAddr(),
                     getLHeight(),
                     getLWidth(),
                     getLStride(),
                     getLXMargin(),
                     getLYMargin());
    xFillPlaneMargin(getMbCbAddr(),
                     getCHeight(),
                     getCWidth(),
                     getCStride(),
                     getCXMargin(),
                     getCYMargin());
    xFillPlaneMargin(getMbCrAddr(),
                     getCHeight(),
                     getCWidth(),
                     getCStride(),
                     getCXMargin(),
...
Void YuvPicBuffer::xFillPlaneMargin(XPel *pucDest,
                                    Int iHeight,
                                    Int iWidth,
                                    Int iStride,
                                    Int iXMargin,
                                    Int iYMargin)
{
    ...
    for(n = 0; n < iHeight; n++)
    {
        ...
        for(m = -iXMargin; m < 0; m++)
            puc[m] = puc[0];

        // right border lum
        //::memset(puc + iWidth,  puc[iWidth - 1], iXMargin*sizeof(XPel));
        for(m = iWidth; m<iWidth+iXMargin; m++)
            puc[m] = puc[iWidth-1];
...
Void YuvPicBuffer::setZero()
{
    ...
    for(n=0,p=getMbLumAddr();n<getLHeight();n++)
    {
        ::memset(p,0x00,getLWidth()*sizeof(XPel));p+=getLStride();
    }

    for(n=0,p=getMbCbAddr ();n<getCHeight();n++)
    {
        ::memset(p,0x00,getCWidth()*sizeof(XPel));p+=getCStride();
    }

    for(n=0,p=getMbCrAddr ();n<getCHeight();n++)
    {
        ::memset(p,0x00,getCWidth()*sizeof(XPel));p+=getCStride();
    }
}
