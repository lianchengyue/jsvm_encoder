...
ErrVal ReconstructionBypass::padRecFrame (Frame*  pcFrame,
                                          ...
                                          UInt  uiSliceId /* = MSYS_UINT_MAX */)
{
    ...
    for(UInt uiMbY = 0; uiMbY < uiFrmHeight; uiMbY++)
    {
        for(UInt uiMbX = 0; uiMbX < uiFrmWidth;  uiMbX++)
        {
            UInt  uiMask  = 0;
            Bool  bIntra  = false;
            pcFrame->getFullPelYuvBuffer()->getYuvBufferCtrl().initMb(uiMbY, uiMbX, bMbAffFrame);

            if(!bMbAffFrame)
            {
                pcMbDataCtrl->getBoundaryMask(uiMbY, uiMbX, bIntra, uiMask, uiSliceId);
                if(!bIntra)
                {
            ...
            {
                pcMbDataCtrl->getBoundaryMask_MbAff (uiMbY, uiMbX, bIntra, uiMask, uiSliceId);
                if (!bIntra)
                {
                    YuvMbBufferExtension  cBuffer;
                    ...
                    if(uiMask)
                    {
                        ...
                        xPadRecMb_MbAff(&cBuffer, uiMask);
                    }
...
ErrVal ReconstructionBypass::xPadRecMb(YuvMbBufferExtension* pcBuffer, UInt uiMask)
{
...
ErrVal ReconstructionBypass::xPadRecMb_MbAff (YuvMbBufferExtension* pcBuffer, UInt uiMask)
{
    ...
    if(!bAvailableCurrTop)
    {
        ...
        xPad8x8Blk_MbAff(pcBuffer, 0, bV0, bV1, bH, bC0, bC1);
    }
    ...
    if(!bAvailableCurrTop)
    {
        ...
        xPad8x8Blk_MbAff(pcBuffer, 1, bV0, bV1, bH, bC0, bC1);
    }
    ...
    if(!bAvailableCurrBot)
    {
        ...
        xPad8x8Blk_MbAff(pcBuffer, 2, bV0, bV1, bH, bC0, bC1);
    }
    ...
    if(!bAvailableCurrBot)
    {
        ...
        xPad8x8Blk_MbAff(pcBuffer, 3, bV0, bV1, bH, bC0, bC1);
    }
                                               ...
                                               Bool bC1)
{
    ...
    if(bDouble)
    {
        xPadBlock_MbAff(pcBuffer, cIdx, bV0, bH, bC0, true,   bFromAbove, bFromLeft);
        xPadBlock_MbAff(pcBuffer, cIdx, bV1, bH, bC1, true,  !bFromAbove, bFromLeft);
    }
    else if(bSwitch)
    {
        xPadBlock_MbAff(pcBuffer, cIdx, bV1, bH, bC1, false, !bFromAbove, bFromLeft);
    }
    else
    {
        xPadBlock_MbAff(pcBuffer, cIdx, bV0, bH, bC0, false,  bFromAbove, bFromLeft);
    }
