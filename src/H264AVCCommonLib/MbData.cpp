...
ErrVal MbData::saveAll(FILE* pFile)
{
    ...
    m_apcMbMotionData[0]->save(pFile);
    m_apcMbMvdData[0]->save(pFile);
    m_apcMbMotionData[1]->save(pFile);
    m_apcMbMvdData[1]->save(pFile);
...
ErrVal MbData::copyMotion(const MbData& rcMbData,
                    UInt    uiSliceId)
{
...
ErrVal MbData::copyIntraPred(MbData &rcMbData)
{
...
// for SVC to AVC rewrite
ErrVal MbData::copyTCoeffs(MbData& rcMbData)
{
    // DECLARATIONS
    TCoeff *piCoeff;
...
Bool MbStatus::canBeUpdated(const SliceHeader& rcSliceHeader)
{
...
ErrVal MbStatus::update(MbDataAccess& rcMbDataAccess)
{
